...
| ◐ Partial | M | L | Mask file save (npy/edf/tif/h5/msk) | MaskToolsWidget.py:104-141 | `.npy` AND `.edf` encode now ported: `render::save::encode_mask_edf` (hand-written fabio-style ASCII `{…}` header + raw `uint8` C-order body, 512-aligned, no new dep) with `MaskToolsWidget::write_edf`/`save_edf`/`save_mask_edf` mirroring the npy path. `MaskToolsWidget::save_mask_dialog` now opens a native rfd save picker (npy/edf filters) and dispatches by extension via the single-owner `resolve_mask_save_format` (missing→npy default, unsupported rejected — faithful to silx `save`). Remaining: TIFF/HDF5/msk encoders need external crates (`tiff`/`hdf5`/fabio), deferred under the no-new-dep pattern |
| ◐ Partial | M | L | Mask file load (npy/edf/tif/h5/msk) | MaskToolsWidget.py:589-629 | `read_npy` AND `read_edf` decode+apply a 2D uint8 mask through the shared `apply_loaded_mask` crop/pad+commit owner (`render::save::decode_mask_edf` reads the fabio `{…}` header for Dim_1/Dim_2/DataType, body taken right after `}`); `load_edf`/`load_mask_edf` mirror the npy file wrappers. `load_mask_dialog` now opens a native rfd open picker (npy/edf) dispatching by extension via `resolve_mask_load_format` (unknown extension rejected, faithful to silx `load`). Missing: TIFF/HDF5/msk decoders (external-crate-bound, deferred), HDF5 dataset selection |
...
| ◐ | H | M | Mask save to file (npy/edf/tif/h5/msk) | `MaskToolsWidget.py:104-141 (save method: edf/tif/npy/h5/msk), 698-785 (_saveMask dialog and format handling)` | `MaskToolsWidget::write_npy` writes a 2D `uint8` `.npy` (silx `save(.., "npy")`); `write_edf`/`save_edf`/`save_mask_edf` now write fabio-style EDF (`render::save::encode_mask_edf`, hand-written, no new dep). The file dialog is now wired (`save_mask_dialog`, native rfd, npy/edf, extension-dispatched). Remaining ◐: TIFF/HDF5/msk encoders (external-crate-bound). Live row 200. |
| ◐ | H | M | Mask load from file (npy/edf/tif/h5/msk) | `MaskToolsWidget.py:589-629 (load method), 630-677 (_loadMask dialog)` | `read_npy` and `read_edf` read a 2D `uint8` mask and apply it through the shared `apply_loaded_mask` owner (crop/pad to the image, committed to undo; silx `load` npy/edf branches); `decode_mask_edf` parses the fabio `{…}` header. The file dialog is now wired (`load_mask_dialog`, native rfd, npy/edf, extension-dispatched). Remaining ◐: TIFF/HDF5/msk decoders (external-crate-bound). |
