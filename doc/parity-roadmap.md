# rsplot → silx parity roadmap

Generated from an 11-agent parity sweep of `silx.gui.plot` against the
rsplot implementation. Scope (per project decision): **silx.gui.plot +
...
**CurvesROIWidget integration (main, not pushed):** ≈225 Done · ~96 open — the last open H row closed: per-curve ROI
...
**CompareImages draggable separator (main, not pushed):** ≈225 Done · ~95 open — 1 M re-baseline row closed (row 220,
◐→✅); the bottom-inventory mode rows (1274/1275) were already ✅, so the headline Done count is unchanged — this closes
the on-plot draggable-separator residual. The split position is now a blue on-plot draggable line marker (silx `__vline`
for `HalfHalf`, `__hline` for `SplitHorizontal`) placed at `split * extent`, recreated when the orientation changes;
dragging it folds the data position back into `split` (silx `__separatorMoved`, clamped `[0,1]` per `__separatorConstraint`),
with the slider kept in sync. This required a structural fix: `MarkerSpec` gained `is_draggable`/`constraint` (silx
`addMarker(draggable=, constraint=)`), now carried by `marker_from_spec` and `marker_spec_from_data` — previously no
high-level API could create a draggable marker (the fields lived only on the mirror `Marker`). Verified by drag-simulation
(`tests/compare_separator.rs`, 3 tests: vertical/horizontal separator drag sets `split`, programmatic `set_split`
repositions the line); GPU render is the shared CompareImages shim.

**StackView Profile3D toolbar (main, not pushed):** ≈226 Done · ~94 open — 1 L row closed (rows 233 + 1291, both ◐→✅).
The row-253 stack-profile extraction (`stack_aligned_profile`/`stack_line_profile`) is now driven by the full silx
`Profile3DToolBar`: new `StackView::show_profile3d_toolbar` (h-line / v-line / line ROI buttons + a 1D/2D `profileType`
toggle, silx `_DefaultImageStackProfileRoiMixIn.setProfileType`), `handle_profile_drag` (a primary drag on the image,
mirroring `ImageView`'s profile drag), and `show_profile` which routes per dimension: 1D → the current frame's profile
in a `ProfileWindow` curve (silx profileType `"1D"`); 2D → the profile stacked over every frame as a colormapped image
in the new `StackProfileWindow` (silx profileType `"2D"`, `core.ImageProfileData(colormap=item.getColormap())` — the
window reuses the stack's own colormap). `StackProfileWindow` mirrors `ProfileWindow`'s viewport/placement but backs a
`Plot2D` image (frame index × profile position, Y not inverted). New public types `StackProfileDimension` +
`StackProfileWindow`; StackView reserves plot ids `id+1`/`id+2` for the two windows. Verified end-to-end by drag-simulation
(`tests/stack_profile_window.rs`, 5 tests: a horizontal drag opens the 2D image window with the hand-verified stacked
row profile, a 1D drag opens the curve window, the dimension toggle hides the inactive window, a line over the volume
yields a profile, and 2D correctly returns nothing in flat-frames mode). 2D requires `set_volume` (a flat-frames stack
has no 3D source — documented, not stubbed).

**Ruler measurement tool wired (main, not pushed):** ≈227 Done · ~93 open — 1 M row closed (row 1279, ◐→✅; the
"Actions, Toolbars, Tool Buttons" section now has 0 ◐). The `RulerToolButton` (checkable button + tested `distance_text`,
silx `buildDistanceText`) is now driven end-to-end on the base `PlotWidget`: `set_ruler_active(true)` enters a line-ROI
draw (`RoiCreate(RoiDrawKind::Line)`, saving the prior interaction mode); `show_inner` adopts each completed line draw
(`response.roi_created`) as *the* ruler line and labels it with its measured length via `distance_text`, recomputing on
edit — a new measurement replaces the previous line (the old is removed; the fresh line is always the last ROI, so its
index is robust under the removal's index shift), and disarming removes the line + restores the saved mode (silx
single-measurement `_RulerROI` / deselect). Faithful to silx, which has no live ROI format-function, so the host
recomputes the label (per the `RulerToolButton` doc-comment). Verified by drag-simulation (`tests/ruler.rs`, 4 tests:
arm enters the line draw, a drag draws a line whose name equals `distance_text` of its own geometry, a second drag
replaces rather than accumulates, disarm removes the line and restores Zoom). rsplot: fmt + clippy + nextest (1335
tests) all pass.

**AlphaSlider item bindings + image-alpha retention (main, not pushed):** ≈228 Done · ~92 open — 1 L row closed (row
1186, ◐→✅; the "Colormap, ColorBar, and AlphaSlider" section now has 0 ◐). The two silx item-binding subclasses are now
wired against the plot model: `ActiveImageAlphaSlider` (silx `getItem()=getActiveImage()`) and `NamedItemAlphaSlider`
(silx `NamedImageAlphaSlider`, an image by legend) seed from the bound image's alpha, disable when no such image exists
(silx "getItem() → None"), and write each slider change back through new `PlotWidget` APIs `image_alpha`/`set_image_alpha`
+ `active_image_alpha`/`set_active_image_alpha`/`active_image_handle`. The enabling structural fix (fix-the-family): the
image's global opacity is now retained in `RetainedItemData::Image` (captured by the single owner `image_spec_retained_data`)
and read back into the rebuilt `ImageSpec` on every re-upload path — level edit, raw-pixel autoscale, median filter — which
previously dropped it to the `ImageSpec::scalar` default of `1.0`. Verified by 9 tests (`tests/alpha_slider_binding.rs`):
alpha survives all three re-upload paths, the per-item alpha API round-trips and clamps, the slider seeds from / disables
without an image, and a kittest slider drag lowers the live image's alpha (image alpha == slider value). Documented model
limitation: silx's curve/scatter *named* bindings stay deferred — rsplot retains a re-applicable per-item alpha only for
scalar images, so those sliders correctly disable rather than drive nothing. rsplot: fmt + clippy + nextest (1344 tests)
all pass.

**Image re-upload preserves interpolation/aggregation too (main, not pushed):** ≈228 Done · ~92 open (no count change —
rows 1157/1160 were already ✅) — structural follow-up closing the rest of the field-drop family the alpha fix exposed.
`RetainedItemData::Image` now also retains `interpolation` + `aggregation` + `aggregation_block`, and a single owner
`ImageDisplayAttrs::apply` is the one place that restores all carried-over display attributes (geometry, alpha,
interpolation, aggregation) onto the rebuilt `ImageSpec::scalar` — so the generic re-upload paths (level edit, autoscale,
median filter, the new `set_image_alpha`) no longer silently reset a non-default `Linear`/`Max` image to the
`scalar()` defaults, and a future re-upload path that calls `apply` cannot drop them. (ImageView was unaffected — it
rebuilds via its own `image_view_image_spec` from separately-held fields.) Verified by a headless round-trip unit test
(`image_display_attrs_round_trip_preserves_every_reupload_field`) asserting every field survives capture→restore. rsplot:
fmt + clippy + nextest (1345 tests) all pass.

**PositionInfo live snapping wired (main, not pushed):** ≈229 Done · ~91 open — 1 M row closed (row 315, ◐→✅; the
"stats-profile-fit-positioninfo" section now has no open snapping row). The two pure cores (`snapping_candidates` +
`snap_to_nearest`) are now driven against a live plot by `PlotWidget::snap_cursor(cursor, mode) -> Option<Snap>`: it builds
the `SnapItem` list from `item_records` (kind via a `PlotItemKind→SnapItemKind` map, `is_item_visible`, `curve_data.symbol`,
active-item), runs the candidate selector, projects each candidate's retained `Curve{x,y}` vertices through the cached
axis-aware `data_to_pixel` (a y2 curve snaps in its own axis), and returns the nearest point within `SNAP_THRESHOLD_DIST`.
Curves, histograms, AND scatters all participate — a base-widget scatter is a symbol-only curve-kind item that retains its
points, so `SCATTER` mode snaps scatter points while `CURVE` mode snaps curve/histogram vertices (kind-filtered by the
mode). `PositionInfo::ui_snapped` reddens the readout in the no-snap state (silx :200) and uses the normal style on a snap
(:288). kittest-verified (`tests/position_info_snap.rs`, 5 tests): on-vertex snap, off-vertex no-snap, DISABLED/SCATTER-mode
filtering, scatter-mode snap (+ CURVE-mode rejecting a scatter), and uncached-transform no-snap. rsplot: fmt + clippy
(`-p rsplot --all-targets`) + nextest (1350 tests) all pass.

**Mask transparency slider UI verified (main, not pushed):** ≈230 Done · ~90 open — 1 S row closed (row 280, ◐→✅). The
mask overlay transparency slider in `MaskToolsWidget::show_toolbar` was already routed through the tested
`set_transparency`/`mask_overlay_lut` cores but its rendered UI was GPU-unverified; now a kittest test
(`tests/mask_transparency_slider.rs`) renders the toolbar (pure egui — the overlay LUT upload is a separate GPU step),
locates the "Transparency" slider by accesskit role+label (the label text and the slider share the label, so role
`SpinButton` disambiguates), drags it to the low end, and asserts the wiring identity — the widget's `transparency()`
(a new getter, symmetric with the existing `set_transparency`) equals the slider's displayed value and drops near the 0.0
minimum. rsplot: fmt + clippy (`-p rsplot --all-targets`) + nextest (1351 tests) all pass.

**ComplexImageView amplitude-range dialog UI verified (main, not pushed):** ≈231 Done · ~89 open — 1 M row closed (row
299, ◐→✅). The `show_amplitude_range_controls` dialog (autoscale checkbox + "Displayed Max." + delta drag-values, silx
`_AmplitudeRangeDialog`) had its composite math + `set/get_amplitude_range_info` accessors tested but the rendered UI was
GPU-unverified; now a kittest+wgpu test (`tests/complex_amplitude_controls.rs`, 3 tests) renders the controls AND the image
each frame (so a range change dispatches the GPU image rebuild through `show`): unchecking "autoscale" seeds the displayed
max from the data max amplitude (5.0) and re-checking clears it to `None` (silx `_autoscaleCheckBoxToggled`); dragging the
now-enabled "Displayed Max." drag-value raises it above the seed; dragging the delta drag-value raises the log-decade count
above the 2.0 default — every change observed through `amplitude_range_info()`. (Drag-values share the slider's accesskit
`SpinButton` role and carry no label, so they are targeted by role + tree order: max first, delta last.) rsplot: fmt +
clippy (`-p rsplot --all-targets`) + nextest (1354 tests) all pass.

**Image per-pixel alpha map (main, not pushed):** ≈232 Done · ~88 open — 1 M row closed (row 254, ☐→✅; duplicate frozen
row 1198 ☐→✅). Per-pixel alpha (silx `ImageData.get/setAlphaData`) ported end-to-end on the scalar GPU path, modeled on
the nan_color shader work. Core: `ImageData` gained `alpha_map: Option<Vec<f32>>` (`with_alpha_map`, length-validated
`w*h`); `ImageSpec`/`ImagePixelsSpec` carry it (`with_alpha_map`, scalar-only — RGBA ignores it as in silx);
`image_data_from_spec` reduces it by the same blocks (mean coverage) when the data is aggregated so it matches the
displayed grid; the `ImageData→ImageSpec` bridge (`image_spec_from_data`) carries it so `add_image_data` keeps it. GPU:
`ImageParams` gained a `has_alpha_map` flag (+ explicit pad → 144 B, std140-tested); the scalar bind-group layout a
binding-5 unfilterable-float alpha texture; `ImagePipeline` a shared 1×1 dummy bound when an image has no map; each tile
uploads its `extract_subgrid` slice to its own R32Float texture. `image.wgsl` hand-rolls the bilinear/nearest alpha
sample (R32Float is unfilterable) under the image's interpolation and multiplies it into the global alpha on both the
normal and nan_color paths. GPU-verified by a headless wgpu readback (`tests/image_alpha_map.rs`): a
left-opaque/right-transparent map over a uniform teal image leaves the left half teal and the right half background,
while an unmasked control render shows teal on both halves (colorbar disabled so its viridis ramp can't pollute the
count). Re-upload preservation of the map follows in the next entry.
rsplot: fmt + clippy (`-p rsplot --all-targets`) + nextest (1355 tests) all pass.

**Image alpha map survives re-upload (main, not pushed):** ≈232 Done · ~88 open (no count change — structural follow-up
closing the rest of the field-drop family, exactly as the interpolation/aggregation follow-up did for the global alpha
fix). `RetainedItemData::Image` now also retains the per-pixel `alpha_map` (captured by the single owner
`image_spec_retained_data`), and the single-owner `ImageDisplayAttrs::apply` — which already restored
geometry/alpha/interpolation/aggregation onto the rebuilt `ImageSpec::scalar` — now restores the alpha map too. So the
generic re-upload paths (level edit, autoscale, median filter, `set_image_alpha`) no longer silently drop a per-pixel
alpha overlay to the `scalar()` default of `None`, and a future re-upload path that calls `apply` cannot drop it. The
attrs bundle owns the map by value (no longer `Copy`) and `apply` borrows it for the rebuilt spec's lifetime; the four
re-upload call sites are unchanged. Verified by the round-trip unit test (now asserting the alpha map survives
capture→restore) and a headless wgpu re-upload readback (`tests/image_alpha_map.rs`: a colormap level edit keeps the
masked right half transparent). rsplot: fmt + clippy (`-p rsplot --all-targets`) + nextest (1356 tests) all pass.

**Mask file save/load dialog (main, not pushed):** ≈232 Done · ~88 open (no count change — rows 326/327/1328/1329 stay ◐
because TIFF/HDF5/msk remain external-crate-bound, but their listed "no file dialog" gap is removed). `MaskToolsWidget`
gained `save_mask_dialog`/`load_mask_dialog` (silx `_saveMask`/`_loadMask`): a native rfd picker with npy/edf filters that
dispatches to the existing hand-written codecs by file extension. The extension→format mapping is one single owner
`mask_format_for_ext`, with `resolve_mask_save_format` (missing extension → npy default, unsupported rejected — faithful
to silx `save`) and `resolve_mask_load_format` (unknown extension rejected — faithful to silx `load` which raises on an
unknown `os.path.splitext` extension). The picker is an untested native shim; the dispatch resolvers are unit-tested (2
tests: save defaults npy + rejects tif/h5, load requires a known extension, both case-insensitive). rsplot: fmt + clippy
(`-p rsplot --all-targets`) + nextest (1358 tests) all pass.

**Round line joins + caps (main, not pushed):** ≈234 Done · ~86 open — 2 L rows closed (1258 joins + 1259 caps, both
☐→✅; the WgpuBackend §13 summary now lists no line-join/cap gap). The polyline was drawn as independent butt-capped
segment quads, so a sharp turn left a wedge gap on the outer side of the join and the two ends were flat. New
`GpuCurve::draw_caps` stamps an antialiased disc of the line width at every polyline vertex (`shaders/linecaps.wgsl`,
reusing the marker bind-group layout + the line's exact 1-px feather AA, sharing the points + per-vertex-color buffers
so a coloured join takes its own vertex's color): the union with the segment quads is a round-joined, round-capped
stroke — silx's pygfx `LineMaterial` default, verified against `~/codes/pygfx/.../wgsl/line.wgsl` (which exposes no
cap/join config and renders rounded). Solid lines only — a disc at every vertex would fill a dash's gaps, so dashed
lines keep butt ends — and zero-width hairlines are skipped. No user-facing butt/round/square *selector* (silx/pygfx
have none on curves; a selector would be a non-silx invention). Honest limitation: pygfx's single-pass unified coverage
has no geometry overlap, whereas the disc double-composites against the segment ends only for *translucent* lines
(opaque lines — the dominant case — are exact, and the bare segments already overlapped at inner corners). GPU readback
verified via the cached `data_to_pixel` (`tests/curve_caps_joins.rs`, 2 tests): a round cap extends ~half the width
beyond the endpoint and its bbox corner is background (rounded, not square); a round join fills the outer wedge of a
right-angle turn (a point beyond both butt segments is painted) without a miter spike past the width. rsplot: fmt +
clippy (`-p rsplot --all-targets`) + nextest (1360 tests) all pass.

**Mask TIFF save/load (main, not pushed):** ≈234 Done · ~86 open (no count change — rows 353/354/1355/1356 stay ◐
because HDF5 + fit2d `.msk` remain, but the TIFF gap in all four is removed). First use of the user's relaxed
new-dependency allowance: added the image-rs `tiff` crate and ported mask TIFF as a single-page grayscale `uint8` image
(silx `MaskToolsWidget` save/load TIFF branch, via fabio/`tifffile`). New single-owner codecs
`render::save::encode_mask_tiff`/`decode_mask_tiff` (decode rejects multi-channel and non-`uint8` samples — a mask is 2D
single-channel `uint8`), with `MaskToolsWidget::write_tiff`/`read_tiff` + `save_tiff`/`load_tiff` +
`save_mask_tiff`/`load_mask_tiff` mirroring the npy/edf wrappers and routing through the shared `apply_loaded_mask`
crop/pad+commit owner. `MaskFileFormat` gained `Tiff`; the single-owner `mask_format_for_ext` maps `tif`/`tiff`
(case-insensitive), so the save/load dialogs gained a TIFF filter + dispatch arm. Verified: codec round-trip +
multi-channel rejection (`render::save` tests), widget read/write round-trip + larger-mask crop (`mask_tools` tests),
and the dispatch tests flipped (`.tif`/`.tiff` now resolve to `Tiff`; `.h5`/`.msk` are the remaining rejects). rsplot:
fmt + clippy (`-p rsplot --all-targets`) + nextest (1364 tests) all pass.

**Mask HDF5 save/load (main, not pushed):** ≈234 Done · ~86 open (no count change — rows 366/367/1368/1369 stay ◐
because fit2d `.msk` remains, but the HDF5 gap in all four is removed, including the "HDF5 dataset selection" sub-gap).
Used the user-chosen pure-Rust `rust-hdf5` crate — a plain build-everywhere dependency (no native libhdf5, no optional
feature, no env vars). New single-owner codecs `render::save::write_mask_hdf5`/`read_mask_hdf5`/`read_mask_hdf5_auto`/
`list_mask_datasets_hdf5`: write a 2D `uint8` dataset (`H5File::create` → `new_dataset::<u8>().shape([h,w])` →
`write_raw` → `close`), read it back (`open` → `dataset` → `ndims()==2` guard → `read_raw`, length-checked), and
enumerate every 2D dataset (`dataset_names()` filtered by `ndims()`, sorted). HDF5 is a random-access container, not a
byte stream, so — unlike npy/edf/tiff — these take a path, not `impl Write`. `MaskToolsWidget` gained `save_h5`/`load_h5`
(load auto-selects the first 2D dataset) + `load_h5_dataset`/`mask_datasets` (the dataset-SELECTION mechanism: enumerate
+ load by an explicit name) + `save_mask_h5`/`load_mask_h5`, all routing through the shared `apply_loaded_mask`
crop/pad+commit owner. `MaskFileFormat::H5`; the single-owner `mask_format_for_ext` maps the six silx
`NEXUS_HDF5_EXT` (`.h5 .nx5 .nxs .hdf .hdf5 .cxi`); both dialogs gained an HDF5 filter + dispatch arm. One divergence
remained at the time of this entry: the interactive Qt "Select a 2D dataset" picker UI is not ported (the underlying
selection mechanism — enumerate + load-by-name — is). (The "fresh standalone file vs silx append" divergence noted here
was later closed — see the "Mask HDF5 save append" entry below.) Verified: widget round-trip + larger-mask crop +
enumerate-then-load-by-name (`mask_tools` tests),
dispatch tests flipped (`.h5`/`.nxs`/`.hdf5` resolve to `H5`; `.msk` the remaining reject). rsplot: fmt + clippy
(`-p rsplot --all-targets`) + nextest (1367 tests); plus the full-workspace gate (Cargo.toml dep change): clippy
(`--workspace --all-targets`) + nextest (1777 tests) all pass.

**Mask fit2d `.msk` save/load (main, not pushed):** ≈234 Done · ~86 open (no count change — rows 366/367/1368/1369 stay
◐, but the mask file save/load now cover ALL FIVE silx formats — npy/edf/tif/h5/msk; at the time of this entry the
remaining sub-gaps were the interactive HDF5 dataset-picker UI and the HDF5 save append "a" mode — the latter has since
been closed, see the "Mask HDF5 save append" entry below). The fabio reference WAS found locally
(`~/mamba/envs/silx-pygfx/.../fabio/fit2dmaskimage.py`, same env as the silx reference), so this is a faithful port, not
a guess. New single-owner codecs `render::save::encode_mask_msk`/`decode_mask_msk` implement the Andy Hammersley fit2d
format: a 1024-byte header (`MASK` magic at bytes 0/4/8/12, `dim1`=width @16 + `dim2`=height @20 as little-endian `u32`,
byte 24 = 1) then one bit per pixel, LSB-first, packed into rows of `((width+31)/32)` 32-bit ints. fit2d masks are
**binary** — every non-zero level collapses to a set bit on write and reads back as 1 (exactly fabio's `data != 0`), a
documented format limitation. `MaskToolsWidget` gained `write_msk`/`read_msk` + `save_msk`/`load_msk` +
`save_mask_msk`/`load_mask_msk` (a byte-stream codec like edf/tiff, routed through the shared `apply_loaded_mask`
owner); `MaskFileFormat::Msk`; `mask_format_for_ext` maps `msk`; both dialogs gained a Fit2D filter + dispatch arm.
Verified BYTE-FOR-BYTE against fabio: a reference `.msk` generated by `fabio.Fit2dMaskImage` on a known 2×5 mask
(`1a00000004000000` body) is reproduced exactly by `encode_mask_msk` (`mask_msk_matches_fabio_reference_bytes`), plus
odd-width bit-padding + bad-magic/short-header rejection (`render::save` tests) and widget binary round-trip +
larger-mask crop (`mask_tools` tests). The dispatch tests flipped (`.msk` now resolves to `Msk`; only a genuinely
unknown extension rejects). rsplot: fmt + clippy (`-p rsplot --all-targets`) + nextest (1372 tests) all pass.

**Mask HDF5 save append "a" mode (main, not pushed):** ≈234 Done · ~86 open (no count change — rows 366/367/1368/1369
stay ◐, but one of the two HDF5 sub-gaps is now closed: only the interactive dataset-picker UI remains). `write_mask_hdf5`
was made faithful to silx's `h5py.File(filename, "a")` open mode: an existing file is opened for append
(`H5File::open_rw`, preserving its other datasets) instead of being truncated (`H5File::create` was the prior
divergence), and an existing dataset at the path is deleted first (`delete_dataset`, silx's overwrite branch) before the
mask is recreated; a missing file is still created. This was verified EMPIRICALLY (rust-hdf5's write-mode
`dataset_names()` does list pre-existing datasets after `open_rw`, and `delete_dataset` works there) with two new tests:
`h5_save_append_preserves_existing_dataset` (seed an unrelated 2D `keep` dataset, save the mask, assert both survive and
read back intact) and `h5_resave_overwrites_existing_mask_dataset` (a second save replaces the mask, not errors on the
duplicate name). rsplot: fmt + clippy (`-p rsplot --all-targets`) + nextest (1374 tests) all pass.

**Mask HDF5 "Select a 2D dataset" picker UI (main, not pushed):** ≈238 Done · ~82 open — 4 rows closed ◐→✅ (366/367 +
duplicates 1368/1369: mask file save/load). This closes the last mask-file-IO sub-gap. silx's `MaskToolsWidget`
`loadAction`/`saveAction` (the Load.../Save... toolbar buttons) and the `DatasetDialog` "Select a 2D dataset" picker are
now ported. `show_toolbar` gained Load.../Save... buttons that open the native rfd file dialog; for HDF5 the dataset
choice then runs through an egui modal (silx `_selectDataset`): a load (`begin_h5_load`) lists the file's 2D datasets to
pick (silx `LoadMode`), a save (`begin_h5_save`) names the target dataset — default `mask`, existing datasets offered as
overwrite shortcuts (silx `SaveMode`) — driving `load_h5_dataset`/the new `save_h5_dataset`. The picker state
(`H5DatasetPicker`) is a pure tested seam: `for_load` seeds the first dataset, `for_save` seeds `mask`, `resolved_path`
applies silx's `lstrip("/")` + empty-rejection rule. The modal itself is GPU-verified with egui_kittest
(`tests/mask_h5_picker.rs`: a load picks the non-default dataset and reads it; a save writes the current mask to the
named dataset). Deviation from silx: the secondary "overwrite existing dataset?" confirmation QMessageBox is not
reproduced (the save modal lists the existing datasets and OK overwrites directly). rsplot: fmt + clippy
(`-p rsplot --all-targets`) + nextest (1379 tests) + doctests all pass; rsdm/adl2rsdm rebuild (public-API add).
...
--no-deps -p rsplot` is clean (0 errors, 0 warnings); rsplot fmt + clippy (`-p rsplot --all-targets`) + nextest (1379
tests) + doctests all pass.

**Section-header count reconciliation (main, not pushed):** no parity-population change (doc-consistency fix). Three
per-section summary headers had drifted from their tables because earlier row-flip commits updated the row glyph but not
the header count: "Items: Image, Marker, Shape, Complex" `11✅ 0◐ 1☐`→`12✅ 0◐ 0☐` (per-pixel alpha row flipped ☐→✅ by
`302fc8b`; its prose still listed per-pixel alpha as a "Remaining gap" — now corrected, both it and concave fill are
closed), "Backend Render" `36✅ 2◐ 2☐`→`38✅ 2◐ 0☐` (line joins+caps flipped ☐→✅ by `9a41c2d`; the `✅` count stays a
prose-summary, only `2☐`→`0☐`), and "Mask tools (image + scatter)" `25✅ 2◐ 0☐`→`27✅ 0◐ 0☐` (mask save/load flipped
◐→✅ by `b878a82`). Verified every section header's ◐/☐ now matches its table (Backend Render's `✅` is a
prose-summary count by design). Dated "After Wave N" progress-log totals above are point-in-time records, left as-is.

**Print-preview page editor (main, not pushed):** 1 L row closed (rows 523/1582, ☐→✅). `PrintPreview`
(`widget/print_preview.rs`): a detachable page editor mirroring silx `PrintPreviewDialog` +
`PrintPreviewToolButton` — a white page on a gray backdrop holding movable, corner-resizable image items, with
the silx default placement (`_getViewBox`: 0.1 offset, 0.9 width, keep-aspect) and the silx keep-ratio resize
math (`_GraphicsResizeRectItem`: pick the smaller growth ratio for uniform scale, 40px bottom-right handle).
`add_image(rgba,w,h,title,comment,pos)` takes the same in-memory RGBA that `render_plot_rgba` (now `pub`, the
single owner of plot→pixels readback) produces for `save_graph`, so a live plot drops onto the page with no
re-snapshot. The pure geometry/composite core is fully tested (`resize_item_rect`, `default_placement`,
`compose_page` nearest-neighbor src-over blit) plus a headless render-path test. **Deviation:** silx `_print`
submits the composed scene to a `QPrinter` device; rsplot has no Qt print device, so the page is composited
to a PNG via `compose_page`/`take_page_png` and written through a Save Page dialog (mirroring silx's own
print-to-file path); live system-printer submission stays with the existing `PrintDialog`. Verified: full
`--workspace` fmt + clippy (`-D warnings`) + nextest (1801 tests) + doctests all pass.

**CompareImages AUTO/SIFT alignment (main, not pushed):** 2 L rows closed (1547 ◐→✅, 1548 ☐→✅; re-baseline row
424 ☐→✅). New pure module `core::sift_align` ports silx `CompareImages.__createSiftData`→`LinearAlign` onto the
pure-Rust `lowe-sift` crate (a new, MIT, rust-version-1.92, `default-features=false` dependency — Lowe's SIFT
detector/descriptor/ratio matcher + `estimate_affine_from_pairs`, the same 6-param affine least squares as silx
`matching_correction`): `sift_auto_align` pads both images to the common `max×max` grid, detects SIFT keypoints,
matches descriptors (Lowe ratio 0.8), fits the affine A→B, and bilinearly warps B onto A's frame (zero border);
`decompose_affine` mirrors `__toAffineTransformation` (`rot=atan2(-b,a)`, `sx=sign(a)·√(a²+b²)`,
`sy=sign(d)·√(c²+d²)`). Wired into `CompareImages` as `CompareAlignment::Auto` (toolbar "auto" button): `show`
runs the registration on the dirty path and caches it; `build_composite` places A padded + the warped B;
`raw_pixel_data` maps the cursor to B through the estimated affine; too few matches fall back to ORIGIN (silx
`__setDefaultAlignmentMode`). **Deviation:** coordinates stay in image (x,y) end-to-end rather than silx's
OpenCL (y,x) kernel swap (same affine, natural axis order); silx's GPU OpenCL SIFT is replaced by the CPU
`lowe-sift` (same Lowe 2004 algorithm). 10 headless core tests + 2 kittest AUTO end-to-end (register a known
shift, featureless fallback). The affine read-out (`getTransformation`, row 1559) and keypoint overlay (row
1561) build on this and land next. Verified: full `--workspace` fmt + clippy (`-D warnings`) + nextest +
doctests all pass (lowe-sift is a new public dependency → workspace-scoped gate).

**CompareImages affine transform read-out (main, not pushed):** 1 L row closed (1576 ☐→✅). `CompareImages::transformation() -> Option<AffineTransformation>` (silx `getTransformation`) exposes the decomposed `(tx,ty,sx,sy,rotation)` from the cached SIFT result — `Some` only while `CompareAlignment::Auto` registration holds, `None` for origin/center/stretch (silx couples it to the SIFT path). The status bar (`show_status_bar`, row 1579) now renders silx's `Align: Translation+Scale+Rotation` summary (`align_summary`: notable-component test at atol 0.01, "No big changes"/"No changes" fallbacks via numpy-isclose defaults) with the per-component px/deg breakdown on hover (`align_tooltip`); silx's `Translation x`-gated-on-`ty` transcription slip is corrected to gate on `tx`. 2 headless summary/tooltip tests + a kittest asserting the (+3,+2) recovery and `None` after a featureless fallback. Composite-views header 29✅ 1◐ 4☐ → 30✅ 1◐ 3☐. Verified: rsplot fmt + clippy (`-p rsplot --all-targets`) + nextest all pass.

**CompareImages keypoint overlay toggle (main, not pushed):** 1 L row closed (1580 ☐→✅). `CompareImages::set_keypoints_visible`/`keypoints_visible` (silx `setKeypointsVisible`/`getKeypointsVisible`, off by default) + `matched_keypoints()` (silx `__matching_keypoints`). `sync_keypoint_overlay` (silx `__updateKeyPoints`) draws the matched keypoints as a `Symbol::Cross` point scatter at their image-A common-grid coords on top of the composite, rebuilt with the composite and cleared when hidden or non-AUTO; toolbar "kp" checkbox wired. silx colours keypoints by scale through a dedicated colormap — rsplot uses one conspicuous colour (a single `add_scatter_with_symbol` colour). kittest toggles the overlay on/off and asserts the matched-keypoint set survives (scatter pixels GPU-unverified). Composite-views header 30✅ 1◐ 3☐ → 31✅ 1◐ 2☐ (remaining: ImageStack URL lazy-load + prefetch). Verified: rsplot fmt + clippy (`-p rsplot --all-targets`) + nextest all pass.

**ImageStack lazy/threaded loading (main, not pushed):** 1 L row closed (1571, ☐→✅). `ImageStack` gains a second mode beside the in-memory `set_frames`: `set_sources(Vec<String>)` + `set_loader(Arc<dyn FrameLoader>)` (silx `setUrls` / `setUrlLoaderClass`). Each slot starts empty — reusing the existing `None`-slot waiting-overlay path (silx `WaitingOverlay`) — and its frame is loaded on a spawned background thread through the pluggable `FrameLoader` trait (silx `UrlLoader.run`, `None` on failure), the result sent back over an `mpsc` channel and drained on the next `ui` pass (silx `_urlLoaded`, queued connection) with `ctx.request_repaint()` to wake the UI. A failed slot is terminal — never re-dispatched — matching silx storing the `None` in `_urlData`. The dispatch/dedup/outcome bookkeeping is a pure `LoadSchedule` (NotRequested/InFlight/Loaded/Failed), unit-tested in isolation (4 tests); `tests/image_stack_lazy.rs` (3 kittest) covers the in-flight→fill (gated loader proves the overlay shows while loading), terminal-failure-not-retried, and browse-loads-the-new-slot paths. `FrameLoader` is re-exported from the crate root. Prefetch radius is row 1573; a concrete HDF5 loader is frozen-inventory row 444. Composite-views header 31✅ 1◐ 2☐ → 32✅ 1◐ 1☐. Verified: rsplot fmt + clippy (`-p rsplot --all-targets`) + nextest + full `--workspace` clippy/nextest/doctests + rustdoc link check all pass.

**ImageStack prefetch queue (main, not pushed):** 1 L row closed (1573, ☐→✅). `ImageStack::set_n_prefetch`/`n_prefetch` (silx `setNPrefetch`/`getNPrefetch`, default 10 = `N_PRELOAD`) set how many neighbours on each side of the current slot to preload. The pure `prefetch_candidates(current, len, n)` computes the load window — current first, then the next `n` slots (nearest first), then the previous `n`, each clamped to `[0, len)` — mirroring silx `setCurrentUrl` loading the current url then `_preFetch(_getNNextUrls(n))` + `_preFetch(_getNPreviousUrls(n))`. `load_candidates` now feeds this window into the same `LoadSchedule`-gated background-dispatch path landed in row 1571, so neighbours preload on their own threads while browsing, deduped against in-flight/loaded slots. 5 pure window tests (middle order, both-ends clamp, zero-radius, empty stack, radius > length) + a kittest (`tests/image_stack_lazy.rs`) asserting current±1 load at radius 1 without navigating, with the out-of-window slots left untouched. Composite-views header 32✅ 1◐ 1☐ → 33✅ 1◐ 0☐ (the section's last ☐ closed; the lone ◐ is row 1572's per-URL add/remove + url-text-in-rows). Verified: rsplot fmt + clippy (`-p rsplot --all-targets`) + nextest + full `--workspace` clippy/nextest/doctests + rustdoc link check all pass.

**ImageStack HDF5 loader (main, not pushed):** 1 frozen-inventory row closed (ImageStack lazy URL/HDF5 loading, ☐→✅). Concrete `Hdf5FrameLoader` (implements the `FrameLoader` trait from row 1571) reads HDF5 frames on demand: a source string is `"<file>::<dataset>"` for a 2D dataset (one frame) or `"<file>::<dataset>::<index>"` for one slice of a 3D `[N,H,W]` stack — only that frame is read off disk via `read_slice`, which is the lazy-path payoff. New `render::save::read_image_hdf5` / `read_image_hdf5_slice` do the read through the pure-Rust `rust-hdf5` crate (the established HDF5 choice) and convert to `f32`. CAVEAT, documented not masked: `rust-hdf5` 0.2's public dataset API surfaces only the element *byte size*, not the datatype *class*, so the read supports float datasets (4-byte f32 / 8-byte f64→f32) and a same-width integer dataset would be reinterpreted — a genuine limitation of the dependency, surfaced in the function/loader docs and rejected for other element sizes. `Hdf5FrameLoader` is re-exported from the crate root; each `load` opens its own file handle so it is safe to call from several prefetch threads. 8 unit tests (read_image_hdf5 f32 roundtrip / f64 cast / non-2D reject; read_image_hdf5_slice frame values / OOB + non-3D reject; loader 2D / 3D-slice / malformed-source-and-missing-file → None). No header tally change (frozen-baseline table). Verified: rsplot fmt + clippy (`-p rsplot --all-targets`) + nextest + full `--workspace` clippy/nextest/doctests + rustdoc link check all pass.

**ImageStack URL table: remove + URL-text rows (main, not pushed):** 1 L row closed (1578, ◐→✅) — the Composite-views section's last ◐. In lazy mode the frame table now shows each row's source/URL text (silx `UrlList`) via `row_label` (loaded frames still in-memory show their frame label), and a per-row "✕" button removes that source (silx `UrlList` delete → `sigUrlRemoved`). `ImageStack::remove_source(index)` (silx `removeUrl`) is the single owner of slot removal: it mutates all four parallel lanes — `sources`, the load `schedule`, and the nav's `frames`/`visible` — in lockstep, so `sources.len() == frames.len()` holds by construction, and `FrameNav::remove` keeps the displayed frame where possible (removal before the current shifts its index down with no rebuild; removing the current points at the next frame clamped to the new last, marked dirty; removing after is index-neutral). 6 pure removal tests (before/current+clamp/after/last-remaining/out-of-range + the `LoadSchedule` lane) + a kittest (`tests/image_stack_lazy.rs`: the table renders all three URLs, removing the middle realigns `sources()`/`len()` and the removed URL disappears from the table). Composite-views header 33✅ 1◐ 0☐ → 34✅ 0◐ 0☐ (section fully closed). Verified: rsplot fmt + clippy (`-p rsplot --all-targets`) + nextest + full `--workspace` clippy/nextest/doctests + rustdoc link check all pass.

**Parity queue exhausted — accepted residuals (main, not pushed):** with the Composite-views section closed (34✅ 0◐ 0☐, row 1578), the actionable silx.gui.plot parity queue is exhausted. Every section is now fully ✅ except two, and their remaining glyphs are accepted residuals, not deferred work — each blocked by a framework/platform constraint that no in-ecosystem port can faithfully close (re-confirming the earlier "truly exhausted" determination at `1a0da5f`, now after the CompareImages SIFT and ImageStack lazy-loading waves landed). The full residual set (6 rows):
- **Backend Render — 38✅ 2◐ 0☐.** (1) addMarker custom font (row 1412): marker labels render in a fixed proportional font — egui `FontId` cannot express silx `QFont` weight/italic without bundling font-variant assets (the marker `bgcolor` half of this row IS done). (2) async GPU stats/histogram (row 1420): pure performance — silx's GPU compute reduction for streaming data is not mirrored; the functional min/max/histogram/autoscale results all exist and are tested on CPU.
- **ROI system — 29✅ 1◐ 0☐.** Mid-gesture `sigInteractiveRoiCreated` (row 1467): rsplot's immediate-mode rubber-band has no persistent mid-draw ROI object to signal, so it emits `DrawingProgress` during the gesture (the faithful analogue) and `RoiCreated` = silx `sigInteractiveRoiFinalized` on draw-finish; the rest of the manager-signal set is distinct and tested.
- **Detailed-table residuals** (each summarised under an otherwise-closed section header): colormap matplotlib-dynamic loading (impossible without a Python/matplotlib dep — the 45-entry static `colorous` catalog is the faithful substitute); print native dialog + printer submission (OS-native, blocking, headlessly unverifiable — the `print_graph()` GPU-readback and the print-PREVIEW page editor are both done); OpenGLAction backend selector (N/A — rsplot is wgpu-only by design).

These six match established-choice exclusion categories (Python-dep / OS-native-blocking-UI / bundled-font-asset / speculative-perf / immediate-mode-structural-deviation) and are documented in their row cells; no remaining row has a clean, headlessly-testable, in-ecosystem pure core left to port. No code changed in this entry (doc-only end-state record).
...
| ✅ Done | M | L | Image per-pixel alpha map | items/image.py:462-500 (get/setAlphaData) | Per-pixel alpha ported end-to-end on the scalar-image GPU path (analogous to the nan_color shader work): `ImageData` gained `alpha_map: Option<Vec<f32>>` (+ `with_alpha_map`, length-validated `w*h`), `ImageSpec`/`ImagePixelsSpec` carry it (`with_alpha_map`, scalar-only — RGBA ignores it as in silx), and `image_data_from_spec` reduces it by the same blocks (mean coverage) when the data is aggregated so it matches the displayed grid. GPU: `ImageParams` gained a `has_alpha_map` flag (+ explicit pad, struct now 144 B, std140-tested), the scalar bind-group layout a binding-5 unfilterable-float alpha texture, and `ImagePipeline` a shared 1×1 dummy bound when an image has no map; each tile uploads its `extract_subgrid` slice of the map to its own R32Float texture. `image.wgsl` samples the alpha texture under the image's nearest/linear interpolation (hand-rolled bilinear, R32Float unfilterable) and multiplies it into the global alpha on both the normal and nan_color paths. The `ImageData→ImageSpec` bridge (`image_spec_from_data`) carries the map so `add_image_data` keeps it. GPU-verified by a headless wgpu readback (`tests/image_alpha_map.rs`): a left-opaque/right-transparent map over a uniform teal image leaves the left half teal and the right half background, while an unmasked control render shows teal on both halves. Re-upload preservation is closed too: `RetainedItemData::Image` retains the map and the single-owner `ImageDisplayAttrs::apply` restores it on every re-upload path (level edit / autoscale / median filter / set_image_alpha), GPU-verified by a level-edit re-upload that keeps the right half masked. |
...
| ✅ Done | L | L | ImageStack lazy URL/HDF5 loading | items/image.py:593-669 | Lazy loading landed (Composite-views rows 1571 lazy + 1573 prefetch): `ImageStack::set_sources`/`set_loader` load each slot on a background thread via the pluggable `FrameLoader`, with a prefetch radius (silx `_preFetch`/`N_PRELOAD`). Concrete `Hdf5FrameLoader` reads `file::dataset` (a 2D dataset) and `file::dataset::index` (one slice of a 3D `[N,H,W]` stack — only that frame is read off disk) through `render::save::read_image_hdf5`/`read_image_hdf5_slice` on the pure-Rust `rust-hdf5` crate; float datasets only (4-byte f32 / 8-byte f64→f32), with the dep's dtype-class limitation (only element byte size is exposed) documented, not silently masked. 8 read/loader unit tests + the 4 lazy/prefetch kittest. |
| ✅ Done | L | L | CompareImages SIFT keypoint alignment | CompareImages.py | `CompareAlignment::Auto` + `core::sift_align` port silx `__createSiftData`→`LinearAlign`: SIFT keypoints/descriptors + Lowe ratio match + 6-param affine least squares + bilinear warp of B onto A's grid, on the pure-Rust `lowe-sift` crate. Frozen rows 1547/1548 carry the detail. |
...
| ✅ Done | M | L | ArcROI interaction sub-modes | items/_arc_roi.py | Both sub-modes' geometry exist as pure cores and the live handle drag now dispatches on the selected mode: PolarMode edits via `Roi::move_edge` (mid/weight/start/end), ThreePointMode via `arc_from_three_points` (faithful `_createGeometryFromControlPoints`: closed-circle / collinear / general branches), `arc_control_points` (derive start/mid/end on the central radius, silx `_updateHandles`/`_updateMidHandle`), and `arc_three_point_drag` (move a control point → re-fit center+radius+angles, weight preserved). `arc_from_two_points` delegates to `arc_from_three_points` (one geometry owner). The live toggle lands: the mode button is the right-click "Interaction mode" submenu (row 1179), and `roi_apply_edge_drag` is the single owner that gates the handle drag — a ThreePointMode Arc exposes the three control-point handles (`arc_three_point_handles`, hit-tested by `arc_three_point_grab`, drawn by `chrome::draw_roi_handles`) and reshapes via `arc_three_point_drag`; every other mode uses the four polar handles + `move_edge`. 8 headless tests (geometry + the mode-gated grab/dispatch/handle-list); the on-screen handle glyphs are GPU-unverified as with every ROI handle draw. |
...
| ✅ Done | L | L | BandROI sub-modes (Bounded/Unbounded) | items/_band_roi.py:172-250 | **Mis-attribution corrected:** silx BandROI has no "rotation sub-mode" — band orientation is already free via the begin/end handles (rsplot `move_edge` Vertex 0/1). silx's actual sub-modes are `BoundedMode` (drawn as the 4-corner polygon, rsplot's default via `band_corners`) and `UnboundedMode` (drawn as three parallel infinite lines). The Unbounded geometry core is `Roi::band_lines() -> BandLines` (faithful `BandGeometry.slope`/`intercept`/`edgesIntercept`), a `Sloped`/`Vertical` sum type so no field carries silx's `inf`-slope dual meaning, spanned to the view by the pure `Roi::band_unbounded_segments(x_range, y_range)`. The live toggle now lands: the mode button is the right-click "Interaction mode" submenu (row 1179/185), and `chrome::draw_roi` swaps the bounded polygon for the three view-spanning lines when `interaction_mode == BandUnbounded`. The spanning geometry is headlessly tested (7 tests) and the live draw is GPU-verified by a headless wgpu render test (`tests/roi_band_unbounded_render.rs`: UnboundedMode reaches the data-area left edge, BoundedMode stays at the band extent). |
...
| ✅ Done | L | M | ROI right-click context menu | tools/roi.py:625-642 (`_feedContextMenu`) | Per-ROI canvas menu (silx `_createMenuForRoi`): ROI-name title + "Make current" (`set_current_roi`) + "Remove" (`remove_roi`); the target ROI is captured via `roi_grab_at` and held in `ui.data` for the menu's lifetime, and the high-level owner applies the signalled intent so CurrentRoiChanged/RoisCleared fire. For Arc/Band ROIs the menu now nests an "Interaction mode" submenu (silx `createMenuForInteractionMode`) over the ported `InteractionModeMixIn` model, applied via `set_roi_interaction_mode` → `RoiInteractionModeChanged`. Closes frozen row 1179. |
| ✅ Done | L | L | ROI save/load from file | CurvesROIWidget.py:194-210 (dictdump) | Data-layer save/load + widget wiring ported: `core::roi_io` `encode_rois`/`decode_rois` (hand-written line-oriented text format, no serde — same manual approach as the `.npy` mask path) round-trip every `Roi` variant plus name/color/line width+style/gap color/fill; the transient `selected` flag is omitted (silx `ROI.toDict` likewise). `save_rois(path,…)`/`load_rois(path)` are the file wrappers (silx `CurvesROIWidget.save`/`load(filename)`). `PlotWidget::save_rois_to_path`/`load_rois_from_path` are the testable widget-level wrappers (load replaces the set via `clear_rois`+`add_managed_roi`, emitting `RoisCleared` + N×`RoiAdded`); `save_rois_dialog`/`load_rois_dialog` open the native rfd file picker like the figure `save_dialog` (GPU/UI-unverified shims). 10 tests (9 `roi_io` round-trip/error cases + a GPU-backed `PlotWidget` save→mutate→load round-trip in `tests/roi_events.rs`). |
...
| ✅ Done | M | L | Mask file save (npy/edf/tif/h5/msk) | MaskToolsWidget.py:104-141 | ALL five silx formats encode now ported: `encode_mask_edf` (fabio-style ASCII `{…}` header + raw `uint8` body, 512-aligned), `encode_mask_tiff` (grayscale `uint8` via the `tiff` crate), `write_mask_hdf5` (2D `uint8` dataset via pure-Rust `rust-hdf5`), and `encode_mask_msk` (Andy Hammersley fit2d binary bit-pack, byte-verified against fabio `Fit2dMaskImage`) — with `write_edf`/`write_tiff`/`write_msk`/`save_h5` + `save_*`/`save_mask_*` wrappers mirroring the npy path. `save_mask_dialog` opens a native rfd save picker (npy/edf/tiff/hdf5/msk filters) and dispatches by extension via the single-owner `resolve_mask_save_format` (missing→npy default, unsupported rejected — faithful to silx `save`). `show_toolbar` now renders silx's `loadAction`/`saveAction` (Load.../Save... buttons). HDF5 save is faithful to silx's "a" mode (`open_rw` an existing file preserving its other datasets + delete-and-recreate the named dataset, else create). The HDF5 "Select a 2D dataset" picker is ported (silx `_selectDataset(SaveMode)`): `begin_h5_save` parks the choice and `show_toolbar`'s egui modal names the target dataset (default `mask`, existing datasets offered as overwrite shortcuts), driving `save_h5_dataset`. Deviation: the secondary "overwrite existing dataset?" QMessageBox is not reproduced (the modal lists the existing datasets and OK overwrites directly). |
| ✅ Done | M | L | Mask file load (npy/edf/tif/h5/msk) | MaskToolsWidget.py:589-629 | ALL five silx formats decode+apply a 2D uint8 mask through the shared `apply_loaded_mask` crop/pad+commit owner: `read_npy`, `read_edf` (fabio `{…}` header), `read_tiff` (single-channel `uint8`), `load_h5` (`read_mask_hdf5`/`read_mask_hdf5_auto` via `rust-hdf5`, `ndims()==2` guarded), and `read_msk` (`decode_mask_msk`, fit2d bit-unpack to a binary mask, byte-verified vs fabio) — with `load_*`/`load_mask_*` wrappers. `load_mask_dialog` opens a native rfd open picker (npy/edf/tiff/hdf5/msk) dispatching by extension via `resolve_mask_load_format` (unknown extension rejected, faithful to silx `load`), reachable from the `show_toolbar` Load... button. The interactive Qt "Select a 2D dataset" picker is ported (silx `_selectDataset(LoadMode)`): `begin_h5_load` enumerates 2D datasets (`mask_datasets`) and parks the choice; `show_toolbar`'s egui modal lists them and drives `load_h5_dataset` on the chosen one. GPU-verified by the egui_kittest modal test (`tests/mask_h5_picker.rs`). |
| ✅ Done | M | S | Mask transparency/alpha slider UI | _BaseMaskToolsWidget.py:554-577 | Wave 7: Transparency slider in `MaskToolsWidget::show_toolbar` (the coloured-overlay toolbar) routes through the tested `set_transparency` (silx `transparencySlider` → `_setMaskColors`: selected level at this alpha, others at half) and re-renders the overlay LUT. Live UI verified (this wave): kittest test (`tests/mask_transparency_slider.rs`) renders `show_toolbar`, drags the rendered "Transparency" slider to its low end, and asserts the widget's `transparency()` (new getter, symmetric with `set_transparency`) equals the slider's displayed value and drops near the 0.0 minimum — proving the slider drives `set_transparency`. N/A for ImageView's mask (rendered as NaN holes, no overlay alpha) |
...
| ✅ Done | M | M | CompareImages vline/hline separator modes | CompareImages.py:124-133, 422-445 | Wave 7: `HalfHalf` is the silx VERTICAL_LINE composite (A left / B right); `CompareMode::SplitHorizontal` = HORIZONTAL_LINE (A top / B bottom) via the pure `split_composite` helper + toolbar button. The split position is now an **on-plot draggable line marker** (silx `__vline`/`__hline`) — a blue draggable `Marker::vline`/`hline` placed at `split * extent`, recreated when the orientation changes; dragging it reads the data position back into `split` (silx `__separatorMoved`), clamped to `[0,1]` (silx `__separatorConstraint`). The slider is kept as a convenience and stays in sync. Verified by drag-simulation (`tests/compare_separator.rs`, 3 tests) |
| ✅ Done | M | M | ComplexImageView amplitude-range dialog | ComplexImageView.py:50-155, items/complex.py:62-82, :199-212 | Wave 7: `ComplexMode::Log10AmplitudePhase` + `amplitude_phase_log_rgba(data, max, delta)` port silx `_complex2rgbalog` (clamp to displayed max `smax`, `log10(\|z\|+1e-20)`, shift by `a.max()-delta`, normalize over `delta` decades) — amplitude → HSV value (rsplot convention). `set_amplitude_range_info(max, delta)` / `amplitude_range_info()` = silx `_setAmplitudeRangeInfo`/`_getAmplitudeRangeInfo`, defaults max=None autoscale + delta=2 (`DEFAULT_AMPLITUDE_DELTA`). Composite math unit-tested. `87cd1d4` adds `show_amplitude_range_controls` (autoscale checkbox + Displayed Max + delta, silx `_AmplitudeRangeDialog`) with the tested `data_max_amplitude` seed. Live UI verified (this wave): kittest+wgpu test (`tests/complex_amplitude_controls.rs`, 3 tests) renders the controls + the image each frame (so the range change dispatches the GPU rebuild via `show`): clicking "autoscale" off seeds the displayed max from the data max (5.0) and re-checking clears it to `None`; dragging the enabled "Displayed Max." drag-value raises it above the seed; dragging the delta drag-value raises the log-decade count above the 2.0 default — all observed through `amplitude_range_info()` |
| ✅ Done | L | L | StackView 3D-profile toolbar | StackView.py:948-951 | Extraction (`StackView::stack_aligned_profile`/`stack_line_profile` over the loaded volume + current perspective, silx `ProfileImageStack{HorizontalLine,VerticalLine,Line}ROI`) now drives the full tool: `show_profile3d_toolbar` (the h-line / v-line / line ROI buttons + a 1D/2D `profileType` toggle, silx `Profile3DToolBar` + `_DefaultImageStackProfileRoiMixIn.setProfileType`); a primary drag on the image runs `handle_profile_drag` → `show_profile`, which routes to the 1D current-frame curve window (`ProfileWindow`, silx profileType `"1D"`) or the new `StackProfileWindow` 2D image of the stacked profile (silx profileType `"2D"`, `core.ImageProfileData(colormap=item.getColormap())`). Verified end-to-end by drag-simulation (`tests/stack_profile_window.rs`, 5 tests: 2D/1D window opens on drag, dimension toggle hides the inactive window, line over volume, 2D needs a volume). 2D requires `set_volume` (flat-frames mode has no stack source) |
...
| ✅ Done | L | L | SaveAction PDF/EPS/JPEG | PlotWidget.py:3232-3242 | All three exported, hand-written with no new dep. EPS/PDF are SVG-style raster-embedding substitutes for silx's matplotlib vector output: `render::save::encode_eps` (PostScript `colorimage`, Y-flip matrix, ASCII-hex body) and `encode_pdf` (single-page `/DeviceRGB` `/ASCIIHexDecode` image XObject with a byte-offset xref). JPEG is a hand-written baseline JFIF encoder (`render::jpeg::encode_jpeg` — T.81 Annex K tables IJG-scaled to quality 90, 4:4:4, canonical Huffman, 0xFF stuffing), the same on-disk container PIL writes for silx. `SaveFormat::Eps`/`Pdf`/`Jpeg` + `from_extension` + dispatch + the native save-dialog filters wired. PNG/PPM/SVG/TIFF/JPEG/EPS/PDF supported; only `.ps` remains unmapped |
...
| ☐ — | L | S | OpenGLAction (backend selector) | actions/control.py | N/A: rsplot is wgpu-only (intentional) |
...
- **Deferred follow-ups** (cross-cluster wiring): arcsinh branch in `image.wgsl`;
  ColormapDialog Stddev3/Percentile need Plot2D raw-pixel access;
  mask on-plot drawing (Plot2D wiring) + Bresenham `draw_line` + mask file I/O.
  (Heart glyph SDF — done: `Symbol::Heart` cardioid SDF in `markers.wgsl`.)
...
- Gate: clippy `-p rsplot --all-targets` clean (== `--workspace`, sole member), **672 tests pass** (+7:
...
- Gate: clippy `-p rsplot --all-targets` clean (printers compiled), **674 tests pass** (+2), doctests ok.
...
- Gate: clippy `-p rsplot --all-targets` clean, **680 tests pass** (+6), doctests ok (no doctest changed).
...
- Gate (main, post-fix): fmt clean, clippy `-p rsplot --all-targets` clean (medians compiled),
...
- **Wave 9 complete**, `main` @ `71f5653`, 730 tests, full-workspace gate green (`-p rsplot` == sole
...
each passing the full per-crate gate (`cargo fmt --all`; `cargo clippy -p rsplot --all-targets -- -D warnings`
clean; `cargo nextest run -p rsplot`; `cargo test --doc -p rsplot`). Closes candidate-wave #1 — the top
...
- **Wave 13 complete**, `main` @ `7495590` (NOT pushed). Per-crate scope only (`-p rsplot` == `--workspace`,
...
commit `-p rsplot`; `--workspace` clippy clean + `nextest` 1861/1861.
...
that default constraint set. rsplot's `PeakModel` estimate→fit path is
...
rsplot has strong coverage of core axis features: linear/log/inverted axes, dual Y2 axis, axis labels with active-curve-label fallback (`resolved_axis_label`), grid modes (major/major+minor/none) with an independent grid color, nice-number tick layout with minor ticks, TIME_SERIES/datetime axes (DST-aware named IANA zones via `TimeZone`), per-axis autoscale (`set_x/y/y2_autoscale`, honored by `reset_zoom_to_data_range`), data-margin expansion ratios for reset-zoom (`DataMargins`, log-aware), axis-visibility toggle (`set_axes_displayed`), axis constraints, keep-aspect-ratio, and auto-zoom-to-data. The silx dirty/replot lifecycle is resolved-not-a-gap (rsplot is immediate-mode with persistent GPU buffers; `DirtyState` is a faithful port, set not consumed). `Plot::data_range()` mirrors silx `getDataRange` and is kept live by `recompute_data_bounds` (every content change pushes the raw per-axis bounds into the cache), so this section is fully covered.
...
| ✅ N/A | L | S | Overlay-only replot optimization | `PlotWidget.py:719-730 (dirty='overlay')` | Resolved W15 — not a gap. rsplot is immediate-mode with persistent GPU buffers (data uploaded on add/update, not per frame); there is no per-frame full upload for dirty='overlay' to skip. Parity achieved differently. |
...
| ✅ | M | L | Limits history (undo/redo stack for zoom history) | `LimitsHistory.py:34-82` | `core/plot.rs` `limits_history: Vec<LimitsHistoryEntry>` (left + optional y2 ranges) pushed before each box-zoom/pan (wheel zoom does NOT push — silx pushes only from the drag-zoom interaction; R1-7) and cleared on every zoom-mode entry (silx `Zoom.__init__`); `zoom_back` pops the most recent (silx `LimitsHistory`, unbounded stack). |
...
| ✅ N/A | L | L | Interaction state machine (ClickOrDrag, StateMachine) | `Interaction.py:87-198, PlotInteraction.py:153-209` | Resolved — not a gap. Implementation structure, not a user feature; rsplot achieves the same interaction behavior with imperative `apply_interaction` dispatch + `DrawState`. Functional parity achieved differently. |
...
## Items: Image, Marker, Shape, Complex  — 12✅ 0◐ 0☐

rsplot implements core image (scalar colormapped + RGBA direct), marker (point/vline/hline), and shape (polygon/rectangle/polyline/hline/vline) drawing. Colormaps support 8 built-in names with linear/log/sqrt/gamma normalization and a 256-entry LUT. Images support origin/scale placement, GPU tiling for large datasets, per-pixel validity masking (NaN→transparent), nearest/linear data interpolation, max/mean/min aggregation downsampling, and per-pixel alpha maps. Markers support point symbols, draggability with horizontal/vertical/custom-callback constraints, text labels with 9-way anchor positioning, and line styling. Shapes support fill, dashed-gap outline styling, and a data-vs-overlay layer flag. Complex images render via `ComplexImageView` (all 7 silx modes), 3D stacks via `ImageStack` (in-memory frame browser), and infinite lines via the `Line` item (`y = slope·x + intercept`). Per-pixel image alpha (GPU alpha texture + shader sample, retained across re-uploads) and concave-polygon fill (triangulated mesh fill) are now both closed — this section has no remaining gaps.

| | P | E | Feature | silx | gap |
|---|---|---|---|---|---|
| ✅ | M | M | Image per-pixel alpha map | `items/image.py:462-500 (ImageData.getAlphaData/setAlphaData, alternative image with alpha)` | DONE (GPU). Closed alongside primary row 254: `ImageData.alpha_map` + `ImageSpec::with_alpha_map` (scalar-only), a binding-5 R32Float alpha texture per tile (shared 1×1 dummy when absent) + a `has_alpha_map` flag in `ImageParams` (struct now 144 B), and an `image.wgsl` alpha-texture multiply into the global alpha on both the normal and nan_color paths, interpolation-matched. GPU-verified by `tests/image_alpha_map.rs` (left-opaque/right-transparent map vs an unmasked control). |
...
| ✅ | L | L | Image aggregation/downsampling modes (max/mean/min) | `items/image_aggregated.py:46-138 (ImageDataAggregated.Aggregation enum with NONE/MAX/MEAN/MIN, _getLevelOfDetails LOD reduction)` | `AggregationMode::{None, Max, Mean, Min}` (gpu_image.rs) + `aggregate_blocks(field, w, h, bx, by, mode)` reduce a field by integer block factors (silx `ImageDataAggregated` MAX/MEAN/MIN); wired as the image spec's `aggregation` + `aggregation_block` fields applied on the display path. Default None matches silx. Reducer tested (max/min/mean/none, odd dims). The aggregation mode + block factors are now retained in `RetainedItemData::Image` and restored (with origin/scale/alpha/interpolation) by the single-owner `ImageDisplayAttrs::apply` on every generic image re-upload path (level edit / autoscale / median filter / alpha-slider), closing a latent reset-to-default the prior per-field restore left open. |
...
| ✅ | L | L | Shape fill concavity (concave polygon fill) | `items/shape.py (silx defers to backend; matplotlib/pygfx fill concave correctly)` | Closed: `core::shape::triangulate_simple_polygon` (ear clipping, orientation-normalized, `N-2` triangles for a simple polygon, defensive fan on degenerate/self-intersecting input) lets the chrome fill `Shape`/`Roi` polygons as a triangulated `egui::Mesh` (`chrome::fill_polygon`) instead of egui's convex-only `Shape::convex_polygon`. Concave polygons now fill to their true outline, matching silx's matplotlib/pygfx backends (which fill concave correctly). Convex shapes (rect, ellipse 27-gon, band quad, diamond marker) are unchanged. 5 headless tests (triangle, convex quad, concave-tiles-exactly-not-hull, CW-input normalization, <3-points). Mesh draw GPU-painter-unverified. |
| ✅ | L | M | Image interpolation mode (nearest vs linear sampling) | `backends/BackendMatplotlib.py:805 (interpolation='nearest' hardcoded; silx backends via matplotlib accept 'nearest'/'bilinear'/etc.)` | `InterpolationMode::{Nearest, Linear}` (gpu_image.rs) is exposed as the `ImageData.interpolation` field (`with_interpolation` setter, default Nearest = silx) and carried into `image.wgsl` via `code()` (0/1) where the shader branches between nearest and bilinear scalar sampling. Mode plumbing tested; on-screen sampling GPU-unverified. The interpolation mode is now retained in `RetainedItemData::Image` and restored by `ImageDisplayAttrs::apply` across every generic image re-upload path (level edit / autoscale / median filter / alpha-slider), so a non-default `Linear` image keeps its sampling instead of silently reverting to `Nearest` on re-upload. |
...
## Backend Render (WgpuBackend vs BackendPygfx)  — 38✅ 2◐ 0☐

The WgpuBackend implements the core Backend trait with substantial coverage of BackendPygfx: additive item methods (addCurve, addImage, addTriangles, addShape, addMarker with the full silx symbol set + drag constraints), state management (limits, axes, colors, margins), and core rendering (transforms, data-to-pixel, picking). `save_graph` exports PNG/PPM/TIFF/SVG (DPI accepted-but-ignored, matching silx). Remaining true gaps: async GPU stats/histogram reduction (functional autoscale/stats/histogram exist on CPU; the GPU-compute performance path is not mirrored). Several silx backend methods — setGraphGrid, setGraphCursor, time-series axis, postRedisplay — live at rsplot's `Plot`/chrome/widget layer rather than the GPU backend: an architectural split, not missing features (marked ✅ N/A below).
...
| ✅ | H | M | Symbol support: circle, square, cross, plus, diamond, etc. | `BackendBase.py:116-126, BackendPygfx.py:85-98 (_SYMBOL_MAP)`, `items/core.py:731 (SymbolMixIn._SUPPORTED_SYMBOLS)` | One unified `items::Symbol` catalog covers the full 18-symbol silx `_SUPPORTED_SYMBOLS` set ('o'/'s'/'x'/'+'/'d'/'.'/','/'\|'/'_'/tickleft/tickright/tickup/tickdown/caretleft/caretright/caretup/caretdown/'♥') plus a Triangle egui-extra. **Markers share this exact catalog with curve vertices** — silx's `Marker` subclasses `SymbolSingleSizeMixIn(SymbolMixIn)`, so there is no separate marker-symbol set (the earlier 7-variant `MarkerSymbol` was a wrong-premise duplicate, now deleted). Curves draw via the GPU SDF shader (`markers.wgsl`, all 18 + heart cardioid); markers draw via the egui painter (`chrome::draw_marker_symbol`, all 18 cases at on-plot sizing); the legend icon painter (`high_level::draw_legend_symbol`) is the fixed-size analogue. |
...
| ✅ | L | L | Line joins: round/bevel/miter for connected segments | `BackendPygfx.py uses pygfx LineMaterial (no explicit join config visible)` | **Round joins** (silx's pygfx `LineMaterial` default — verified against `~/codes/pygfx/.../line.wgsl`, which has no cap/join config and renders rounded). `GpuCurve::draw_caps` stamps an AA disc of the line width (`shaders/linecaps.wgsl`, sharing the marker bind-group layout) at every polyline vertex, filling the outer-wedge gap a sharp turn left between the butt-capped segment quads. Solid lines only (a disc at every vertex would fill a dash's gaps; dashes keep butt ends). pygfx's single-pass unified coverage avoids any geometry overlap; the disc approach double-composites at the segment overlap only for *translucent* lines (opaque lines — the dominant case — are exact, and the bare segments already overlapped at inner corners). GPU readback verified: `tests/curve_caps_joins.rs::solid_curve_has_round_joins_at_turns`. |
| ✅ | L | L | Line caps: butt, round, square at line endpoints | `BackendPygfx.py uses pygfx LineMaterial` | **Round caps** via the same per-vertex AA disc (`GpuCurve::draw_caps`): the disc at each endpoint extends the stroke into a half-disc, the pygfx `LineMaterial` default. A user-facing butt/round/square *selector* is not ported — silx/pygfx expose no such API on curves, so a selector would be a non-silx invention. GPU readback verified the cap extends ~half the width beyond the endpoint and is rounded, not square: `tests/curve_caps_joins.rs::solid_curve_has_round_caps_at_endpoints`. |
| ✅ N/A | L | L | Grid on/off: toggle major/both grid display | `BackendBase.py:543-549, BackendPygfx.py:2754-2756` | Resolved differently — grid display is `Plot::grid` (`GraphGrid::None`/`Major`/`MajorAndMinor`), drawn by the chrome layer. silx exposes `setGraphGrid` on the backend; rsplot keeps it on `Plot`/chrome (the GPU backend draws data, not the grid) — an architectural split, not a missing feature. |
...
## Colormap, ColorBar, and AlphaSlider  — 15✅ 0◐ 0☐

rsplot implements colormaps with a 45-entry catalog covering silx's full named set (gray, reversed gray via `reversed()`, red, green, blue, temperature, jet, hsv, the viridis family, plus 30+ colorous gradients standing in for matplotlib's catalog), all 5 silx normalizations (Linear/Log/Sqrt/Gamma/Arcsinh), NaN-color configuration, and the three silx autoscale modes (MinMax/Stddev3/Percentile, with configurable percentile bounds via `AutoscaleMode::range`). The `ColorBarWidget` is a self-contained drawer with min/max labels, log/linear ticks, a rotated legend, and both vertical/horizontal orientations; the `ColormapDialog` adds name/normalization/gamma/autoscale/range controls plus a data-distribution histogram; an `AlphaSlider` widget exposes 0-255 opacity, and its item-binding subclasses (`ActiveImageAlphaSlider` / `NamedItemAlphaSlider`) drive a plot image's alpha — backed by image alpha now being retained and preserved across every image re-upload path. `Colormap` supports custom Nx4 LUTs (`from_colors`/`set_lut`), an editable flag, copy/equality, and `set_from`. The one documented model limitation is that the named alpha binding covers images only (curve/scatter carry no re-applicable per-item alpha here, so those sliders disable for them, matching silx's no-item behavior).

| | P | E | Feature | silx | gap |
|---|---|---|---|---|---|
| ✅ | H | L | Colormap catalog names | `/Users/stevek/codes/silx/src/silx/gui/colors.py:444-455, /Users/stevek/codes/silx/src/silx/math/colormap.py:53-65` | `ColormapName` is a 45-entry static catalog (`ALL`) covering silx's entire documented named set (`setName`: "at least" `gray`, `reversed gray`, `temperature`, `red`, `green`, `blue`, `jet`, `viridis`, `magma`, `inferno`, `plasma`). `Gray`/`Red`/`Green`/`Blue` are exact `single_channel_ramp`s, `Temperature` is transcribed channel-by-channel from `_create_colormap_lut`'s `arange` slices (`temperature_lut`, tested), `reversed gray` = `Colormap::reversed()` (LUT reverse), and `Jet`/`Hsv` + 30+ colorous gradients (Blues/Greens/Warm/Cool/Cubehelix/diverging/cyclical) stand in for matplotlib's catalog. Residual (not a gap in silx's contract): rsplot ships a fixed static catalog rather than resolving arbitrary matplotlib names at runtime — it carries no matplotlib dependency (analogous to silx loading the viridis family from bundled `.npy` resources). |
...
| ✅ | L | M | AlphaSlider widget | `/Users/stevek/codes/silx/src/silx/gui/plot/AlphaSlider.py:86-300` | The base `AlphaSlider` (alpha_slider.rs) is a 0..=255 opacity slider exposing `u8`/`f32` alpha (`value/255`, silx `valueChanged`) with `AlphaSliderOrientation` and a `ui() -> Response`; the item-binding subclasses are now wired against the plot model: `ActiveImageAlphaSlider` (silx `ActiveImageAlphaSlider`, `getItem()=getActiveImage()`) and `NamedItemAlphaSlider` (silx `NamedImageAlphaSlider`, an image by legend) seed from the bound image's alpha, disable when there is no such image (silx "getItem() → None disables"), and write each change back through the new `PlotWidget::set_image_alpha`/`set_active_image_alpha`. The enabling structural change: image alpha is now retained in `RetainedItemData::Image` (single owner `image_spec_retained_data`) and preserved across every re-upload path — level edit, raw-pixel autoscale, median filter — which previously reset it to the `ImageSpec::scalar` default of `1.0`. Verified by 9 tests (`tests/alpha_slider_binding.rs`): retention across all three re-upload paths, the per-item alpha API, slider seeding/disabling, and a kittest drag writing alpha back to the image. Documented model limitation (not a silx-contract gap closed here): silx's curve/scatter *named* bindings are not mirrored — rsplot retains a re-applicable per-item alpha only for scalar images (a curve bakes opacity into its color; a scatter retains no data), so those named sliders correctly disable rather than drive nothing. |
| ✅ | L | M | Custom colormap registration | `/Users/stevek/codes/silx/src/silx/math/colormap.py:158-176, /Users/stevek/codes/silx/src/silx/gui/colors.py:343-386` | Per-instance custom LUTs (`Colormap::from_colors(&[[u8;4]], vmin, vmax)`, `with_lut`/`set_lut`, silx `setColormapLUT`) AND a process-global named-LUT registry: `register_colormap(name, &[[u8;4]], cursor_color: Option)` (silx `registerLUT`/`register_colormap` — resamples via `resample_lut`, overriding allowed, empty colors → `false`), `Colormap::from_registered(name, vmin, vmax)` resolves a registered name to its LUT (silx `Colormap(name=…)`), `registered_colormaps()` (silx `get_registered_colormaps`), `registered_colormap_cursor_color()` (silx `get_colormap_cursor_color`). Backed by a `OnceLock<RwLock<BTreeMap>>`, the rsplot analogue of silx's module-level `_AVAILABLE_LUTS`/`_COLORMAP_CACHE`. 5 headless tests. |
| ✅ | L | S | Colormap copy/comparison/serialization | `/Users/stevek/codes/silx/src/silx/gui/colors.py:399-423, 960-1050` | `Colormap` derives Clone (copy) + PartialEq (`__eq__`) and has `set_from` (silx `setFromColormap`, editable-guarded). State round-trip now ported in `core::colormap_io` (silx `saveState`/`restoreState`): `encode_colormap`/`decode_colormap` (hand-written line-oriented text, no serde — same approach as `roi_io` and the `.npy` mask path) + `save_colormap`/`load_colormap` file wrappers. Because rsplot's `Colormap` stores a resolved LUT and keeps no name (unlike silx, which serializes the name + registry), the state serializes the LUT itself → lossless `decode(encode(c)) == c` for catalog, `from_colors`, and registered LUTs alike; `lut` is the only required line, every other field defaults when absent (silx version-gated `restoreState` defaults), unknown keys ignored. 8 headless tests. |
...
## ROI system (creation, editing, manager, statistics)  — 29✅ 1◐ 0☐

rsplot implements a ROI core with all 11 silx shape kinds (Rect, HRange, VRange, Point, Line, Polygon, Cross, Circle, Ellipse [oriented], Arc, Band), supporting on-plot interactive creation for every kind (Wave 12: arm a draw shape, draw it, the ROI is appended), edge dragging via handle-detection, and whole-ROI translate — all in pure data space. Wave 13 unified `plot.rois` to a single `Vec<ManagedRoi>` (geometry + appearance), closing the visual/behavioral gap: per-ROI color, on-canvas naming/labels, selection highlighting (single-owner `Plot::set_current_roi` + `sigCurrentRoiChanged`), handle-symbol roles, and style properties (line width, style, gap color, fill) all reach the live plot; `Roi::contains()` is implemented for every variant; and ROI statistics (curve raw/net counts + area, image/curve stats tables via CurvesROIWidget/ROIStatsWidget) landed in W15. The per-shape interaction sub-modes are now wired end-to-end: the right-click "Interaction mode" submenu picks the mode, the Band Bounded↔Unbounded toggle drives the live render (`chrome::draw_roi`), and the Arc PolarMode↔ThreePointMode toggle drives the live handle drag (`roi_apply_edge_drag` routes to `move_edge` vs `arc_three_point_drag`, with the handle set gated to match). The manager is now a real table widget (`show_roi_manager`, silx `RegionOfInterestTableWidget`): per-row editable name + geometry make-current selector + remove, with per-ROI stats columns supplied by the sibling `CurvesRoiWidget`/`RoiStatsWidget` tables — GPU-verified by an egui_kittest interaction test (`tests/roi_manager_render.rs`). ROI persistence/load is complete: `core::roi_io` text serialization + `PlotWidget::save_rois_to_path`/`load_rois_from_path` + native `save_rois_dialog`/`load_rois_dialog`.
...
| ✅ | H | L | Manager ROI list as table widget (ROITable/CurvesROIWidget) | `CurvesROIWidget.py:62-400, ROITable:452-860` | `PlotWidget::show_roi_manager` is now an `egui::Grid` table (silx `RegionOfInterestTableWidget`): one row per ROI with an **editable name** (silx label column → `set_roi_name`), the **geometry as a make-current selector** (silx row selection → `set_current_roi`/`sigCurrentRoiChanged`, highlighted when current), and a remove button, plus the add-kind / clear-all controls. Per-row edits are collected under the immutable `rois` borrow and applied through the owner APIs after it ends (events fire, current-index stays consistent). The complementary **per-ROI stats columns** (min/max/sum/mean/integral, raw/net counts/area, COM/coords) are provided by the sibling `CurvesRoiWidget`/`RoiStatsWidget` tables (rows 1160-1162). GPU-verified by a headless egui_kittest interaction test (`tests/roi_manager_render.rs`): the table renders a row per ROI and clicking a row's geometry selector makes that ROI current. |
...
| ✅ | M | M | ROI handle symbols ('+', 's', 'o', custom glyphs) | `items/_roi_base.py:735-759 (addHandle/addLabelHandle/addTranslateHandle), PolygonROI:1244-1274` | `draw_roi_handles` draws the silx role symbols: `+` for translate/center, square `s` for vertex/edge. The `o` is silx `PolygonROI._handleClose` (a transient role-"user" creation marker from the `RegionOfInterestManager` path); rsplot creates polygons via the `PlotInteraction` draw path and shows its faithful close target instead — the `SelectPolygon.updateFirstPoint` fill=None box. |
...
| ◐ | M | M | Manager signals (sigRoiAdded, sigRoiChanged, sigCurrentRoiChanged, sigInteractiveRoiCreated/Finalized) | `tools/roi.py:331-371` | Now distinct: `RoiAdded { index }` (silx `sigRoiAdded`) fires from every add — `add_roi`/`add_managed_roi` and the interactive create (which emits `RoiAdded` then `RoiCreated`, silx's `sigRoiAdded`→`sigInteractiveRoiFinalized` order). `RoiChanged` (silx `sigRoiChanged`) is now geometry-change-only; `CurrentRoiChanged` = `sigCurrentRoiChanged`; `RoiCreated` = `sigInteractiveRoiFinalized` (draw finished). `RoiAboutToBeRemoved { index }` (silx `sigRoiAboutToBeRemoved`) is now emitted by `remove_roi` *before* the removal (so a listener can still read the dropped ROI); `RoisCleared` is reserved for `clear_rois` (clear-all) — removing the prior dual meaning where a single removal also emitted `RoisCleared`. Regression-tested in `tests/roi_events.rs` (3 cases: about-to-be-removed-before-removal + not-RoisCleared, clear-all, out-of-range no-op). Remaining ◐: no separate `sigInteractiveRoiCreated` (mid-gesture) — rsplot builds the ROI only on draw-finish, so there is no mid-draw ROI object to signal (the rubber-band emits `DrawingProgress` instead). |
...
| ✅ | M | S | ROI context menu (edit, delete, mode selection) | `tools/roi.py:625-642` | Right-clicking a ROI on the canvas opens a per-ROI menu (silx `_createMenuForRoi`): ROI-name title, "Make current" (→ `set_current_roi`, silx `sigCurrentRoiChanged`), "Remove" (→ `remove_roi`), and — for `InteractionModeMixIn` ROIs (Arc/Band) — a nested "Interaction mode" submenu (silx `createMenuForInteractionMode`) of the ROI's `available_interaction_modes`, the current one radio-checked, each tooltipped with its silx description, applied via `set_roi_interaction_mode` → `RoiInteractionModeChanged`. The target ROI is captured at right-click via `roi_grab_at` and held in `ui.data` for the menu's lifetime; the canvas only signals intent (`Interaction.roi_removed`/`roi_make_current`/`roi_set_interaction_mode`) and the high-level owner performs the mutation + event. "edit" = on-canvas handle drag (Wave 12, separate). The mode-change API + event are GPU-backed-tested in `tests/roi_events.rs`; the menu render itself is GPU-unverified. |
| ✅ | L | L | ArcROI kind (circular arc with start/end angle) | `items/_arc_roi.py (full file)` | `Roi::Arc { center, inner/outer radius, start/end angle }` exists; Wave 12 adds on-plot creation (`RoiDrawKind::Arc`, 2-point line drag) and PolarMode handle editing (mid/weight/start/end). The ThreePointMode geometry is ported too — `arc_from_three_points` (faithful `_createGeometryFromControlPoints`), `arc_control_points`, and `arc_three_point_drag` (control-point reshape, re-fits the circumcircle, preserves weight); `arc_from_two_points` delegates to it. The mode is selectable via the right-click "Interaction mode" submenu + `set_roi_interaction_mode` (row 1179), and the live handle drag now dispatches on it (`roi_apply_edge_drag`): a ThreePointMode Arc shows/hit-tests/reshapes the three control points, PolarMode the four polar handles. 8 headless tests for the Arc sub-mode model + dispatch; handle glyphs GPU-unverified. |
| ✅ | L | L | BandROI kind (rotatable rectangular band) | `items/_band_roi.py (full file)` | `Roi::Band { begin, end, width }` exists with on-plot creation (`RoiDrawKind::Band`) and silx's width constraints (`width>=0`, width handles constrained to the band normal). Orientation is free via the begin/end handles (silx has no separate rotate handle). silx's real sub-modes are `BoundedMode` (polygon, rsplot default) and `UnboundedMode` (three infinite lines) — the Unbounded geometry is `Roi::band_lines()` (faithful `BandGeometry` line equations, `Sloped`/`Vertical` sum type), spanned to the visible view by the pure `Roi::band_unbounded_segments(x_range, y_range)` (3 view-clipped segments, tested headlessly). The mode is selectable via the right-click "Interaction mode" submenu + `set_roi_interaction_mode` (row 1179), and `chrome::draw_roi` now swaps the bounded polygon for the three view-spanning lines when the band's `interaction_mode` is `BandUnbounded`. The swap is GPU-verified by a headless wgpu render test (`tests/roi_band_unbounded_render.rs`): UnboundedMode's lines reach the data-area left edge while BoundedMode's polygon stays at the band's `x∈[5,9]` extent. |
...
| ✅ | L | M | ROI interaction modes (select, edit, focus constraints) | `items/_roi_base.py:135-230 (InteractionModeMixIn, RoiInteractionMode enum)` | Wave 12 adds a select-vs-create mode toggle and, within editing, both edge-drag and whole-ROI translate (`RoiGrab::Edge`/`RoiGrab::Translate`). The silx `InteractionModeMixIn` data model is now ported: `RoiInteractionMode` (Arc `3 points`/`Polar`/`Translation`, Band `Bounded`/`Unbounded`, with silx labels + descriptions), `Roi::available_interaction_modes`/`default_interaction_mode`, and a validated `ManagedRoi::interaction_mode`/`set_interaction_mode` that rejects modes foreign to the kind — so the invariant `mode ∈ available_modes` holds by construction; `ManagedRoi::new` seeds the silx default per kind. 5 headless tests. The mode is now selectable through the right-click "Interaction mode" submenu + `PlotWidget::set_roi_interaction_mode` (→ `RoiInteractionModeChanged`, row 1179). The Band `Bounded`↔`Unbounded` mode drives the live render (row 1181 — `chrome::draw_roi` swaps polygon ↔ three view-spanning lines), and the Arc `3 points`↔`Polar` live handle drag now dispatches on the selected mode (row 1180): `roi_apply_edge_drag` is the single owner that routes a grabbed handle to `arc_three_point_drag` (ThreePointMode, three control-point handles) or `Roi::move_edge` (PolarMode + every other ROI), with the handle set (`arc_three_point_handles`/`arc_three_point_grab`) and `chrome::draw_roi_handles` gated to match. Headlessly tested (grab + dispatch + handle list); the on-screen handle glyphs and the menu render are GPU-unverified. |
...
| ✅ | L | M | ROI save/load from file (dictdump format) | `CurvesROIWidget.py:194-210, roi save/load methods` | Data layer + widget wiring ported: `core::roi_io` `encode_rois`/`decode_rois` (hand-written line-oriented text, no serde — mirrors the `.npy` mask path) round-trip every `Roi` variant + name/color/line width+style/gap color/fill (the transient `selected` is dropped, as silx `ROI.toDict` does), with `save_rois`/`load_rois` file wrappers (silx `CurvesROIWidget.save`/`load(filename)`). `PlotWidget::save_rois_to_path`/`load_rois_from_path` are the testable widget-level wrappers — load replaces the live set via `clear_rois` then one `add_managed_roi` per ROI, so it emits `RoisCleared` + N×`RoiAdded`; `save_rois_dialog`/`load_rois_dialog` open the native rfd file picker, mirroring the figure `save_dialog` (row 236). 10 tests: 9 `roi_io` round-trip/error cases + a GPU-backed `PlotWidget` save→mutate→load round-trip in `tests/roi_events.rs` asserting the replaced set + the cleared/added events. The rfd dialogs are GPU/UI-unverified shims, as with every native dialog. |
...
## Mask tools (image + scatter)  — 27✅ 0◐ 0☐

rsplot's mask tools are comprehensive. `MaskToolsWidget` (image) and `ScatterMaskWidget` (scatter points by index) both implement multi-level masks (0 unmasked, 1..=255 levels), the full drawing-tool set (pencil/eraser brush with Bresenham line interpolation + variable width, rectangle, polygon, ellipse), threshold masking (below/between/above), NaN masking, per-level invert/clear, undo/redo history (silx `BaseMask`), per-level color overrides, a transparency slider, the Mask/Unmask + Ctrl toggle, and a pan/browse (no-draw) tool — all rendered by `show_toolbar` and faithful to silx `_BaseMaskToolsWidget`. The colormapped overlay (`mask_overlay_lut`) renders the masked pixels. Mask file save/load now cover ALL five silx formats — `.npy`, `.edf`, `.tif`/`.tiff`, `.h5` (HDF5, pure-Rust `rust-hdf5`), and fit2d `.msk` (binary bit-pack, byte-verified against fabio) — via `write_*`/`read_*` + `save_*`/`load_*` wrappers sharing the `apply_loaded_mask` crop/pad owner. `show_toolbar` renders silx's Load.../Save... actions opening the native rfd file dialog; HDF5 save is faithful to silx's "a" append mode (preserve other datasets, overwrite only the named one), and the "Select a 2D dataset" picker is ported as an egui modal (`begin_h5_load`/`begin_h5_save` → modal → `load_h5_dataset`/`save_h5_dataset`), GPU-verified by `tests/mask_h5_picker.rs`. The only unreproduced silx detail is the secondary "overwrite existing dataset?" confirmation QMessageBox.
...
| ✅ | H | M | Mask save to file (npy/edf/tif/h5/msk) | `MaskToolsWidget.py:104-141 (save method: edf/tif/npy/h5/msk), 698-785 (_saveMask dialog and format handling)` | ALL five formats: `write_npy` (2D `uint8` `.npy`), `write_edf` (fabio-style EDF), `write_tiff` (grayscale `uint8` TIFF), `save_h5`/`save_h5_dataset` (HDF5 dataset via pure-Rust `rust-hdf5`), `write_msk` (fit2d binary, `encode_mask_msk` byte-verified vs fabio) — each with `save_*`/`save_mask_*` wrappers. The file dialog is wired (`save_mask_dialog`, native rfd, npy/edf/tiff/hdf5/msk, extension-dispatched) behind the `show_toolbar` Save... button (silx `saveAction`). HDF5 save is faithful to silx's "a" mode (`open_rw` append + delete-and-recreate the named dataset, else create) and the "Select a 2D dataset" save picker is ported (`begin_h5_save` → `show_toolbar` modal → `save_h5_dataset`; default `mask`, existing datasets as overwrite shortcuts). GPU-verified by `tests/mask_h5_picker.rs`. Deviation: the secondary "overwrite existing dataset?" QMessageBox is not reproduced. Live row 200. |
| ✅ | H | M | Mask load from file (npy/edf/tif/h5/msk) | `MaskToolsWidget.py:589-629 (load method), 630-677 (_loadMask dialog)` | ALL five formats: `read_npy`, `read_edf`, `read_tiff`, `load_h5` (`read_mask_hdf5` via `rust-hdf5`, `ndims()==2` guarded), `read_msk` (`decode_mask_msk`, fit2d bit-unpack, byte-verified vs fabio) read a 2D `uint8` mask and apply it through the shared `apply_loaded_mask` owner (crop/pad to the image, committed to undo). Dataset selection: `mask_datasets` enumerates + `load_h5_dataset` loads a named dataset; the "Select a 2D dataset" picker is ported (`begin_h5_load` → `show_toolbar` egui modal listing the datasets → `load_h5_dataset`). The file dialog is wired (`load_mask_dialog`, native rfd, npy/edf/tiff/hdf5/msk, extension-dispatched) behind the `show_toolbar` Load... button (silx `loadAction`). GPU-verified by `tests/mask_h5_picker.rs`. |
...
| ✅ | H | S | Mask level spinbox (1-255) with tooltip | `_BaseMaskToolsWidget.py:583-592` | `MaskToolsWidget::show_toolbar` renders a `Slider` over `level` 1..=255 (silx `levelSpinBox`; rsplot uses a slider rather than a spinbox); the selected level is the highlighted/edited one. |
...
| ✅ | H | M | Zoom Back (limits history pop) | `actions/control.py:101-122` | `actions/control.rs` `zoom_back` pops `Plot::limits_history` (unbounded stack, pushed before each box-zoom/pan; wheel does not push, R1-7); wired to the `ToolbarIcon::ZoomBack` button (silx `ZoomBackAction` / `getLimitsHistory().pop()`). |
...
| ✅ | L | M | RulerToolButton (distance measurement tool) | `tools/RulerToolButton.py:83-180+` | `widget::tool_buttons::RulerToolButton` — a checkable button (silx `setCheckable`/`isChecked`/`toggled`) plus the pure distance formatter `distance_text` (silx `buildDistanceText`: Euclidean norm of the two data endpoints, formatted `" .1f"` + `px`), tested headlessly. The host integration is now wired end-to-end on `PlotWidget`: `set_ruler_active(true)` enters a line-ROI draw (`RoiCreate(Line)`, saving the prior mode), and `show` labels the drawn line ROI with its measured length via `distance_text`, recomputed live; a new measurement replaces the previous ruler line (silx single-measurement `_RulerROI`), and disarming removes the line + restores the prior mode. Verified by drag-simulation (`tests/ruler.rs`, 4 tests: arm enters the draw, a drag draws a line whose name equals its own length, a second drag replaces it, disarm removes it + restores the mode). |
...
| ✅ N/A | L | S | OpenGL Backend Toggle | `actions/control.py:685-753` | N/A — silx `OpenGLAction` switches between its Matplotlib and OpenGL backends; rsplot has a single wgpu backend, so there is no backend to toggle (the gap note itself says "no backend selection UI needed"). |
...
## Composite views (ImageView/ScatterView/StackView/CompareImages/ComplexImageView/ImageStack)  — 34✅ 0◐ 0☐

rsplot implements all six composite views. ImageView has side histograms, axis sync, the `RadarView` position-overview, profile-toolbar integration (embedded + popup `ProfileWindow`), an intensity-histogram API (`active_image_histogram`), and aggregation modes. ScatterView has value-coloured scatter, an X/Y/Data/Index position-info panel, the `ScatterMaskWidget` per-point mask tool, and line-profile extraction across the scatter (`line_profile`, silx `ScatterProfileToolBar`). StackView browses frames plus 3D volumes (perspective/axis selection, auto-transposition, per-dimension labels, and per-frame max/mean/min block aggregation). CompareImages offers A/B, vertical-line (`HalfHalf`) and horizontal-line (`SplitHorizontal`) separators, A−B subtract, red-blue-gray RGB composite modes (`RedBlueGray`/`RedBlueGrayNeg`), and origin/center/stretch/**auto** image alignment for differently-shaped A and B (`CompareAlignment`: zero-pad to a common max grid, bilinear-resample B to A's shape, or SIFT-register B onto A). The SIFT `AUTO` path is now wired (`core::sift_align` on the pure-Rust `lowe-sift` crate — Lowe SIFT keypoints + ratio matching + affine least squares + bilinear warp, mirroring silx `__createSiftData`→`LinearAlign`), so the prior "needs a heavy computer-vision dependency" gap is closed. ComplexImageView renders the full `ComplexMode` set with a mode toolbar and amplitude-range controls. ImageStack provides a frame browser with a selectable table, per-frame visibility, and a waiting overlay. CompareImages also has a coordinate/value status bar (`show_status_bar`, silx `CompareImagesStatusBar`). Remaining CompareImages SIFT residuals are the affine-transform read-out (`getTransformation`, row 1559) and the keypoint-visibility scatter overlay (row 1561), both built on the now-landed registration; plus the interactive UI residuals on ScatterView's scatter-profile and StackView's full 3D-profile toolbar (◐, extraction cores done). ImageStack now also loads lazily: `set_sources`/`set_loader` load each slot on a background thread through the pluggable `FrameLoader` (silx `UrlLoader`/`setUrlLoaderClass`), `set_n_prefetch` preloads a radius of neighbours as you browse (silx `_preFetch`/`N_PRELOAD`), the concrete `Hdf5FrameLoader` reads 2D datasets and 3D-stack slices on demand (frozen-inventory row), and the URL table shows each source with a per-row remove (silx `UrlList`/`removeUrl`). The Composite-views section is now fully closed (34✅ 0◐ 0☐).
...
| ✅ | M | M | CompareImages: vertical line separator (vline mode) | `CompareImages.py:124-133,397-407` | `CompareMode::HalfHalf` (silx `VisualizationMode.VERTICAL_LINE`): a left/right split at the `split` fraction — A left of the separator, B right. The split is set by an on-plot draggable `Marker::vline` (silx `__vline`) at `split * width`, dragging it folds back into `split` (silx `__separatorMoved`), or by the slider. `tests/compare_separator.rs`. |
| ✅ | M | M | CompareImages: horizontal line separator (hline mode) | `CompareImages.py:135-144,397-407` | `CompareMode::SplitHorizontal` (silx `VisualizationMode.HORIZONTAL_LINE`): a top/bottom split at the `split` fraction — A above the separator, B below. The split is set by an on-plot draggable `Marker::hline` (silx `__hline`) at `split * height`, or by the slider. `tests/compare_separator.rs`. |
...
| ✅ | L | L | ScatterView: scatter profile toolbar | `ScatterView.py:141,306` | Extraction wired: `core::scatter_viz::scatter_line_profile` + `ScatterView::line_profile(start, end, n_points)` port silx `ScatterProfileToolBar::_computeProfile` (`tools/profile/rois.py:737-762`) — `numpy.linspace` samples interpolated through the scatter's Delaunay mesh (`LinearNDInterpolator` via `delaunay` + `interpolate`), `None` outside the convex hull (silx `NaN`), returning a `ScatterLineProfile`. Pure extraction headlessly tested (affine field exact along the line, outside-hull all-None, <3-points all-None). Side profile-plot **display now wired**: `ScatterView` owns a `ProfileWindow` (reserving plot id `id + 1`); `show_line_profile(start, end, n)` samples the profile, converts it to a value-vs-distance curve via the pure `ScatterLineProfile::distance_value_curve` (Euclidean distance from the first sample; out-of-hull `None` → `NaN` gaps), pushes it through the new `ProfileWindow::set_profile_curve`, and opens the side window — drawn each frame by `ScatterView::show`. Data path headlessly tested (`tests/scatter_profile.rs`: in-hull segment opens the window, out-of-hull / no-data are no-ops; `distance_value_curve` distances + NaN gaps); the window pixels render in their own egui viewport (GPU-unverified). Interactive **draw tool wired**: `set_profile_mode(true)` arms the line-profile tool — it switches the plot to `Select` (so a primary drag neither pans nor box-zooms, and with no ROI under the cursor nothing else consumes it) and `handle_profile_drag` maps the drag's start/current pixels to data via the plot transform and samples `show_line_profile` (silx `__nPoints = 1024`) each dragged frame; disarming restores `Zoom` and closes the window (silx deselect). End-to-end verified by a headless egui_kittest **drag simulation** (`tests/scatter_profile.rs`: a simulated drag across the hull opens the profile window; disarming closes it). |
| ✅ | L | L | StackView: profile 3D toolbar (extract profiles over all frames) | `StackView.py:84,948-951` | Full tool wired over the row-253 extraction cores. `show_profile3d_toolbar` draws silx `Profile3DToolBar`'s h-line/v-line/line ROI buttons plus a 1D/2D `profileType` toggle; a primary drag (`handle_profile_drag` → `show_profile`) routes to the 1D current-frame `ProfileWindow` curve (silx `"1D"`) or the new `StackProfileWindow` 2D image of the stacked profile (silx `"2D"`, `core.ImageProfileData` colormapped with the stack's own colormap). Drag-simulation verified (`tests/stack_profile_window.rs`, 5 tests). 2D requires a loaded volume (`set_volume`); flat-frames mode has no stack source. |
...
| ✅ | L | L | CompareImages: alignment modes (origin/center/stretch/auto) | `tools/compare/core.py:66-72` | `CompareAlignment` {Origin,Center,Stretch,Auto} (silx `AlignmentMode`): `set_images` now takes separate `(w,h)` shapes for A and B, and the pure `align_compare_images` places them on a common grid per mode — Origin/Center zero-pad to `max×max` via `margin_image` (top-left, or silx `size//2−shape//2` centering), Stretch keeps A and bilinearly resamples B to A's shape via `rescale_array` (silx `__rescaleArray`/`BilinearImage.c_funct`, corner-aligned, edge-clamped). `raw_pixel_data` remaps the cursor per mode (`compare_aligned_coords`, AUTO via the estimated affine). Toolbar alignment selector wired (orig/ctr/fit/auto). **AUTO now wired** (row 1548): SIFT registration warps B onto A's grid; on too few keypoints it falls back to ORIGIN (silx `__setDefaultAlignmentMode`). 8 headless align tests + the SIFT core tests + 2 kittest AUTO end-to-end (register-shift, featureless-fallback). GPU render is the shared CompareImages shim. |
| ✅ | L | L | CompareImages: SIFT keypoint detection and alignment | `CompareImages.py:45,350,517-518` | `core::sift_align` (`sift_auto_align`) ports silx `__createSiftData`→`LinearAlign` on the pure-Rust `lowe-sift` crate (Lowe SIFT detector/descriptor/ratio matcher + `estimate_affine_from_pairs` = silx `matching_correction`'s 6-param affine least squares): detect SIFT keypoints on both images (padded to the common `max×max` grid), match descriptors (Lowe ratio 0.8), fit the affine A→B, and bilinearly warp B onto A's frame (zero border). `decompose_affine` mirrors `__toAffineTransformation` (`rot=atan2(-b,a)`, `sx=sign(a)·√(a²+b²)`, `sy=sign(d)·√(c²+d²)`), kept in image (x,y) order rather than silx's OpenCL (y,x) kernel swap. Wired as `CompareAlignment::Auto` (row 1547). 10 headless core tests (normalize, pad, decompose identity/rot+scale, warp identity/translation, recover-known-translation, reject too-few-matches/bad-lengths) + 2 kittest. Keypoint scatter overlay + affine read-out are rows 1561/1559. |
...
| ✅ | L | L | ImageStack: lazy frame loading from URLs | `ImageStack.py:148-200` | `ImageStack::set_sources` + `set_loader` (silx `setUrls`/`setUrlLoaderClass`) switch to lazy mode: each slot starts empty (the existing `None`-slot waiting overlay = silx `WaitingOverlay`) and its frame is loaded on a spawned background thread through the pluggable `FrameLoader` trait (silx `UrlLoader.run`, returning `None` on failure), the result drained back on the UI thread and filled in place (silx `_urlLoaded`, queued connection); a failed slot is terminal (not retried, matching silx storing the `None` in `_urlData`). The pure `LoadSchedule` (NotRequested/InFlight/Loaded/Failed) gates dispatch + dedups (4 unit tests); `tests/image_stack_lazy.rs` (3 kittest) covers in-flight-overlay→fill, terminal-failure-not-retried, browse-loads-the-new-slot. Prefetch radius is row 1573; the concrete HDF5 loader is frozen-inventory row 444. |
| ✅ | L | L | ImageStack: URL selection table and browser | `ImageStack.py:65-128,337-392` | The selectable frame table + browser (slider + per-row toggleable visibility) is implemented (silx `_ToggleableUrlSelectionTable` + `UrlList`, `image_stack.rs`). In lazy mode the rows show the source/URL text (silx `UrlList`) via `row_label`, and a per-row "✕" removes that source: `remove_source(index)` (silx `removeUrl`/`sigUrlRemoved`) is the single owner of slot removal — it drops the source, frame, visibility, and load-state lanes in lockstep (so `sources.len() == frames.len()` holds by construction) and keeps the displayed frame where possible (`FrameNav::remove`: a removal before the current shifts its index down, removing the current points at the next clamped frame, removing after is index-neutral). 6 pure removal tests (before/current/after/last/OOB + the schedule lane) + a kittest (rows show the URLs; removing the middle realigns the lanes and the URL disappears). |
| ✅ | L | L | ImageStack: prefetch queue for smooth browsing | `ImageStack.py:251-293` | `ImageStack::set_n_prefetch`/`n_prefetch` (silx `setNPrefetch`/`getNPrefetch`, default 10 = `N_PRELOAD`) set the prefetch radius. The pure `prefetch_candidates(current, len, n)` window (current first, then the next `n`, then the previous `n`, each clamped) mirrors silx `setCurrentUrl` loading the current url then `_preFetch(_getNNextUrls(n))` + `_preFetch(_getNPreviousUrls(n))`; `load_candidates` feeds it into the same `LoadSchedule`-gated background-dispatch path (row 1571), so neighbours preload on their own threads as you browse. 5 pure window tests (middle/both-ends-clamp/zero-radius/empty/radius>len) + a kittest asserting current±1 load at radius 1 without navigating while out-of-window slots stay untouched. |
...
| ✅ | L | M | StackView: aggregation mode for multi-band frames | `StackView.py:301-305` | `StackView::aggregation`/`set_aggregation` + a None/Max/Mean/Min combo and per-axis `bx`/`by` block factors in `show_frame_controls` (silx `AggregationModeAction` → `_stackItem.setAggregationMode`); each displayed frame routes through an `ImageSpec` carrying the calibrated origin/scale + `aggregation`/`aggregation_block`, driving the GPU `aggregate_blocks` reducer. silx's `Aggregation` enum is exactly NONE/MAX/MEAN/MIN (the prior "mean/sum" note was stale — there is no SUM) and rsplot matches it. Reuses the tested `aggregate_blocks` primitive; StackView needs a wgpu RenderState no test builds, so the wiring is GPU-unverified (parity with ImageView row 1291). |
| ✅ | L | M | CompareImages: affine transformation tracking | `CompareImages.py:880-889` | `CompareImages::transformation() -> Option<AffineTransformation>` (silx `getTransformation`) returns the decomposed `(tx,ty,sx,sy,rotation)` affine — but ONLY from the SIFT/AUTO path: it reads the cached `sift_auto_align` result, so it is `Some` exactly while `CompareAlignment::Auto` registration has succeeded and `None` for origin/center/stretch (silx sets `__transformation = None` in `__updateData`, reassigns only in `__createSiftData`). The status bar now shows the silx `Align: Translation+Scale+Rotation` summary (`align_summary`, atol-0.01 notable test) with the per-component px/deg breakdown on hover (`align_tooltip`); silx's `Translation x`-gated-on-`ty` slip is corrected to gate on `tx`. 2 headless summary/tooltip tests + kittest asserting the (+3,+2) recovery and `None` after fallback. Affine is plain least squares (silx `matching_correction`, no RANSAC — outlier-sensitive). |
...
| ✅ | L | S | CompareImages: keypoint visibility toggle | `CompareImages.py:346-359` | `CompareImages::set_keypoints_visible`/`keypoints_visible` (silx `setKeypointsVisible`/`getKeypointsVisible`, off by default) + `matched_keypoints()` (silx `__matching_keypoints`). `sync_keypoint_overlay` (silx `__updateKeyPoints`) draws the matched keypoints as a point scatter at their image-A common-grid coords on top of the composite, rebuilt with the composite and cleared when hidden / non-AUTO. Toolbar "kp" checkbox wired. silx colours keypoints by scale via a dedicated colormap; rsplot uses one conspicuous colour (`Symbol::Cross`). kittest toggles the overlay on/off and asserts the matched-keypoint set; the scatter pixels render GPU-unverified. |
| ✅ | L | S | CompareImages: status bar with coordinate/value info | `CompareImages.py:191-193` | `CompareImages::show_status_bar` (silx `CompareImagesStatusBar`): the cursor data coordinate (tracked from the pointer event in `show`) plus the raw A / B pixel value under it via `CompareImages::raw_pixel_data` — which remaps the display coordinate to each raw image per the alignment mode (`compare_aligned_coords`) before the bounds-checked `compare_pixel_at` lookup (silx `getRawPixelData`) — formatted as silx `_formatData` `%f`. The silx affine-transform label is now wired (`Align: …` summary + per-component hover, shown only in the SIFT/AUTO path where `getTransformation` is `Some` — row 1576). Lookup + format core tested; status-bar render GPU/UI-unverified. |
...
| ✅ | L | L | Print preview: send plot to printable page with movable/resizable rect | `PrintPreviewToolButton.py:24-120` | `PrintPreview` (`print_preview.rs`) ports silx's `PrintPreviewDialog`: a detachable page editor that places the plot snapshot on a printable page where it can be dragged and corner-resized (`resize_item_rect`, faithful to silx `_GraphicsResizeRectItem` keep-ratio/free), with Clear All / Remove / Zoom± / keep-aspect / Save Page. `add_image` seeds the silx default print geometry (`_getViewBox`: 0.1 offset, 0.9 width, keep-aspect). Deviation: the final device print needs a non-portable `QPrinter`; the no-dep analogue is **Save Page** → `compose_page` PNG (silx's own print-to-file path), and system-printer submission is the separate `PrintDialog` (silx `PrintAction`, row 470). 12 tests; render path exercised headlessly. |
