...
| ◐ Partial | M | L | Mask file load (npy/edf/tif/h5/msk) | MaskToolsWidget.py:589-629 | `MaskToolsWidget::read_npy` decodes+applies a 2D uint8 `.npy` mask (crop/pad to image, commits undo); missing: file dialog, EDF/TIFF/HDF5/msk formats, HDF5 dataset selection |
...
| ◐ | H | S | Data range computation (getDataRange for bounds auto-expand) | `PlotWidget.py:908-918, resetZoom:3331-3350` | `core/plot.rs` `DataRange` (silx `_PlotDataRange`, each member `(min,max)` or `None`) + `data_range()`/`set_data_range()` cache (silx `getDataRange`); `reset_zoom_to_data_range` consumes a `DataRange`. PARTIAL: the high-level widget computes bounds and calls `reset_zoom_to_data_range` directly (`high_level.rs:6552`/`:11151`) without populating the `data_range` cache, so `Plot::data_range()` returns `None` members (silx keeps the cache live via `_updateDataRange`). |
...
## Items: Curve, Histogram, Scatter  — 13✅ 1◐ 0☐

siplot covers curve rendering (solid/dashed lines, single symbols, error bars, fill + baseline) and per-vertex colors, with correct line-style scaling and custom dash patterns. All 17 silx point symbols are implemented (only the `'♥'` Heart glyph is unported). Histograms render as filled step curves with left/center/right alignment tracking (`HistogramAlign`/`histogram_edges`) and filled-region picking (`pick_histogram`: bbox + searchsorted bin + baseline≤y≤value). Scatter supports all silx visualization modes — Points, Solid (Delaunay), RegularGrid, IrregularGrid, and BinnedStatistic (2D binning with mean/count/sum over a rows×cols grid) — plus per-point alpha, and mode-specific picking. Active-curve highlight styling is applied at render (Wave 10) via silx's `getCurrentStyle` per-field merge. Remaining ◐: IRREGULAR_GRID picking maps to nearest-point because that mode renders as an interpolated image, not silx's vertex-indexed triangle mesh (a render redesign needing sign-off, row 152).
...
| | P | E | Feature | silx | gap |
|---|---|---|---|---|---|
| ◐ | H | L | addMarker: point/hline/vline marker with symbol, text, line constraints | `BackendBase.py:211-267, BackendPygfx.py:1397-1465` | Markers (point/hline/vline) with symbol + text + drag constraints are implemented: `MarkerConstraint` (None/Horizontal/Vertical) + `apply_constraint` replace silx's Qt drag-filter callback (data-driven). Residual: marker label text has no background rect (bgcolor) or custom font. |
...
## Colormap, ColorBar, and AlphaSlider  — 11✅ 4◐ 0☐

siplot implements colormaps with 8 catalog entries (Viridis, Inferno, Magma, Plasma, Cividis, Turbo, Greys, Spectral), all 5 silx normalizations (Linear/Log/Sqrt/Gamma/Arcsinh), NaN-color configuration, and the three silx autoscale modes (MinMax/Stddev3/Percentile, with configurable percentile bounds via `AutoscaleMode::range`). The `ColorBarWidget` is a self-contained drawer with min/max labels, log/linear ticks, a rotated legend, and both vertical/horizontal orientations; the `ColormapDialog` adds name/normalization/gamma/autoscale/range controls plus a data-distribution histogram; an `AlphaSlider` widget exposes 0-255 opacity. `Colormap` supports custom Nx4 LUTs (`from_colors`/`set_lut`), an editable flag, copy/equality, and `set_from`. Remaining ◐: the full silx colormap catalog (gray/jet/hsv/temperature + matplotlib names), a global named-LUT registry, the active-item alpha-binding slider subclasses, and state serialization (needs serde).
...
| ◐ | H | M | Mask save to file (npy/edf/tif/h5/csv/msk) | `MaskToolsWidget.py:104-141 (save method: edf/tif/npy/h5/msk), 698-785 (_saveMask dialog and format handling)` | `MaskToolsWidget::write_npy` writes a 2D `uint8` `.npy` (silx `save(.., "npy")`). Remaining ◐: no file dialog and no EDF/TIFF/HDF5/csv/msk encoders. Live row 200. |
| ◐ | H | M | Mask load from file (npy/edf/tif/h5/csv/msk) | `MaskToolsWidget.py:589-629 (load method), 630-677 (_loadMask dialog)` | `MaskToolsWidget::read_npy` reads a 2D `uint8` `.npy` mask and applies it (crop/pad to the image, committed to undo; silx `load` npy branch). Remaining ◐: no file dialog and no EDF/TIFF/HDF5/csv/msk decoders. |
...
## Actions, Toolbars, Tool Buttons  — 27✅ 3◐ 1☐

siplot ships a rich unified toolbar via `show_toolbar()` / `show_toolbar_with()`, with `ToolbarResponse` exposing every state change. Implemented actions: Reset Zoom, Zoom In/Out, Zoom Back (limits-history pop), Select/Pan/Zoom modes, X/Y axis autoscale toggles, X/Y log scale, X/Y invert, Major/Minor grid, Keep-aspect, Show-axis, Crosshair, Curve Style cycle, Save (PNG/PPM/SVG/TIFF + CSV), Copy (clipboard), Print, Median Filter (1D/2D), Pixel-Intensity Histogram, and the Data-Aggregation mode selector (None/Max/Mean/Min). Colormap and colorbar have dedicated widgets (`ColormapDialog`, `ColorBarWidget`); profile/symbol tool buttons exist (`ProfileToolButton`, `SymbolToolButton`); `LimitsWidget` exposes editable axis-limit fields. silx's separate Qt composite toolbars (Interactive/Output/Curve/Image) are delivered as one immediate-mode `show_toolbar` + standalone widgets — a UI-paradigm difference, not missing features. Remaining true gaps: the `RulerToolButton` distance-measurement tool (☐); a `PanWithArrowKeysAction` enable/disable toggle (arrow-key pan itself works — ◐); the optional zoom-axes submenu (◐); and silx's plot-wide lines/points curve-style boolean cycle vs siplot's concrete-`LineStyle` cycle (◐). The OpenGL-vs-Matplotlib backend toggle is N/A (single wgpu backend).
...
| ◐ | L | M | Pan with Arrow Keys Toggle | `actions/control.py:603-625` | Arrow-key panning is implemented (`interaction.rs` `apply_pan` + `PanDirection`; see the Interaction row). Residual: no `PanWithArrowKeysAction`-style toggle to enable/disable it — arrow-key pan is always on. |
...
## Composite views (ImageView/ScatterView/StackView/CompareImages/ComplexImageView/ImageStack)  — 22✅ 2◐ 10☐

siplot implements all six composite views. ImageView has side histograms, axis sync, the `RadarView` position-overview, profile-toolbar integration (embedded + popup `ProfileWindow`), an intensity-histogram API (`active_image_histogram`), and aggregation modes. ScatterView has value-coloured scatter, an X/Y/Data/Index position-info panel, and the `ScatterMaskWidget` per-point mask tool. StackView browses frames plus 3D volumes (perspective/axis selection, auto-transposition, per-dimension labels). CompareImages offers A/B, vertical-line (`HalfHalf`) and horizontal-line (`SplitHorizontal`) separators, and A−B subtract. ComplexImageView renders the full `ComplexMode` set with a mode toolbar and amplitude-range controls. ImageStack provides a frame browser with a selectable table, per-frame visibility, and a waiting overlay. Remaining true gaps concentrate in CompareImages' image-alignment tooling — SIFT keypoint detection + visibility toggle, composite RGB modes, alignment modes (origin/center/stretch/auto), affine-transform tracking, and a coordinate/value status bar — plus ScatterView scatter-profile, StackView per-frame aggregation and full 3D-profile toolbar (◐), and ImageStack's URL-backed lazy loading / prefetch (file-IO, out of scope).
...
| ◐ | M | L | Stats context framework (data masking, bounds clipping, ROI filtering) | `stats/stats.py:143-600` | Bounds clipping (`StatScope::{All, OnLimits{x_range,y_range}}` + `Stats::for_curve`/`for_image`) AND ROI-bounds filtering (`Stats::for_curve_roi(xs, ys, from, to)`, plus arbitrary-shape per-ROI stats via `roi_stats`) cover silx `_StatsContext`'s viewport-clip and ROI paths. Remaining ◐: no per-pixel data-mask context (silx `_StatsContext` mask kwarg). |
