...
## Composite views (ImageView/ScatterView/StackView/CompareImages/ComplexImageView/ImageStack)  — 24✅ 2◐ 8☐

siplot implements all six composite views. ImageView has side histograms, axis sync, the `RadarView` position-overview, profile-toolbar integration (embedded + popup `ProfileWindow`), an intensity-histogram API (`active_image_histogram`), and aggregation modes. ScatterView has value-coloured scatter, an X/Y/Data/Index position-info panel, and the `ScatterMaskWidget` per-point mask tool. StackView browses frames plus 3D volumes (perspective/axis selection, auto-transposition, per-dimension labels, and per-frame max/mean/min block aggregation). CompareImages offers A/B, vertical-line (`HalfHalf`) and horizontal-line (`SplitHorizontal`) separators, and A−B subtract. ComplexImageView renders the full `ComplexMode` set with a mode toolbar and amplitude-range controls. ImageStack provides a frame browser with a selectable table, per-frame visibility, and a waiting overlay. CompareImages also has a coordinate/value status bar (`show_status_bar`, silx `CompareImagesStatusBar`). Remaining true gaps concentrate in CompareImages' image-alignment tooling — SIFT keypoint detection + visibility toggle, composite RGB modes, alignment modes (origin/center/stretch/auto), affine-transform tracking — plus ScatterView scatter-profile, StackView's full 3D-profile toolbar (◐), and ImageStack's URL-backed lazy loading / prefetch (file-IO, out of scope).
...
| ✅ | L | S | CompareImages: status bar with coordinate/value info | `CompareImages.py:191-193` | `CompareImages::show_status_bar` (silx `CompareImagesStatusBar`): the cursor data coordinate (tracked from the pointer event in `show`) plus the raw A / B pixel value under it via `CompareImages::raw_pixel_data` / the pure `compare_pixel_at` (silx `getRawPixelData` ORIGIN alignment, `raw[int(y)][int(x)]`, bounds-checked), formatted as silx `_formatData` `%f`. silx's alignment-transform label is N/A — siplot has no image-alignment modes (rows 1283/1295). Lookup + format core tested; status-bar render GPU/UI-unverified. |
