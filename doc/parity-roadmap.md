...
replaces rather than accumulates, disarm removes the line and restores Zoom). siplot: fmt + clippy + nextest (1335
tests) all pass.
...
limitation: silx's curve/scatter *named* bindings stay deferred — siplot retains a re-applicable per-item alpha only for
scalar images, so those sliders correctly disable rather than drive nothing. siplot: fmt + clippy + nextest (1344 tests)
all pass.
...
(`image_display_attrs_round_trip_preserves_every_reupload_field`) asserting every field survives capture→restore. siplot:
...
filtering, scatter-mode snap (+ CURVE-mode rejecting a scatter), and uncached-transform no-snap. siplot: fmt + clippy
(`-p siplot --all-targets`) + nextest (1350 tests) all pass.
...
minimum. siplot: fmt + clippy (`-p siplot --all-targets`) + nextest (1351 tests) all pass.
...
`SpinButton` role and carry no label, so they are targeted by role + tree order: max first, delta last.) siplot: fmt +
clippy (`-p siplot --all-targets`) + nextest (1354 tests) all pass.
...
siplot: fmt + clippy (`-p siplot --all-targets`) + nextest (1355 tests) all pass.
...
masked right half transparent). siplot: fmt + clippy (`-p siplot --all-targets`) + nextest (1356 tests) all pass.
...
tests: save defaults npy + rejects tif/h5, load requires a known extension, both case-insensitive). siplot: fmt + clippy
(`-p siplot --all-targets`) + nextest (1358 tests) all pass.
...
right-angle turn (a point beyond both butt segments is painted) without a miter spike past the width. siplot: fmt +
clippy (`-p siplot --all-targets`) + nextest (1360 tests) all pass.
...
and the dispatch tests flipped (`.tif`/`.tiff` now resolve to `Tiff`; `.h5`/`.msk` are the remaining rejects). siplot:
fmt + clippy (`-p siplot --all-targets`) + nextest (1364 tests) all pass.
...
dispatch tests flipped (`.h5`/`.nxs`/`.hdf5` resolve to `H5`; `.msk` the remaining reject). siplot: fmt + clippy
(`-p siplot --all-targets`) + nextest (1367 tests); plus the full-workspace gate (Cargo.toml dep change): clippy
...
unknown extension rejects). siplot: fmt + clippy (`-p siplot --all-targets`) + nextest (1372 tests) all pass.
...
duplicate name). siplot: fmt + clippy (`-p siplot --all-targets`) + nextest (1374 tests) all pass.
...
submits the composed scene to a `QPrinter` device; siplot has no Qt print device, so the page is composited
...
The WgpuBackend implements the core Backend trait with substantial coverage of BackendPygfx: additive item methods (addCurve, addImage, addTriangles, addShape, addMarker with the full silx symbol set + drag constraints), state management (limits, axes, colors, margins), and core rendering (transforms, data-to-pixel, picking). `save_graph` exports PNG/PPM/TIFF/SVG (DPI accepted-but-ignored, matching silx). Remaining true gaps: async GPU stats/histogram reduction (functional autoscale/stats/histogram exist on CPU; the GPU-compute performance path is not mirrored). Several silx backend methods — setGraphGrid, setGraphCursor, time-series axis, postRedisplay — live at siplot's `Plot`/chrome/widget layer rather than the GPU backend: an architectural split, not missing features (marked ✅ N/A below).
