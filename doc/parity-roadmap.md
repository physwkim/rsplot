...
| ◐ | L | L | ImageStack: URL selection table and browser | `ImageStack.py:65-128,337-392` | The selectable frame table + browser (slider + per-row toggleable visibility) is implemented (silx `_ToggleableUrlSelectionTable` equivalent, `image_stack.rs`). Lazy URL-backed loading now lands frames on demand (row 1571), so the "held in memory only" residual is closed. Remaining: per-URL add/remove (silx `removeUrl`) and showing the source/URL text in the table rows (rows currently label by frame index / loaded label). |
