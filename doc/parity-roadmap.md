...
`NEXUS_HDF5_EXT` (`.h5 .nx5 .nxs .hdf .hdf5 .cxi`); both dialogs gained an HDF5 filter + dispatch arm. Two divergences
documented honestly: save writes a fresh standalone file (silx appends into an existing file in "a" mode), and the
interactive Qt "Select a 2D dataset" picker UI is not ported (the underlying selection mechanism — enumerate +
load-by-name — is). Verified: widget round-trip + larger-mask crop + enumerate-then-load-by-name (`mask_tools` tests),
...
◐, but the mask file save/load now cover ALL FIVE silx formats — npy/edf/tif/h5/msk; the only remaining sub-gaps are the
interactive HDF5 dataset-picker UI and the HDF5 save append "a" mode). The fabio reference WAS found locally
...
| ◐ Partial | M | L | Mask file save (npy/edf/tif/h5/msk) | MaskToolsWidget.py:104-141 | ALL five silx formats encode now ported: `encode_mask_edf` (fabio-style ASCII `{…}` header + raw `uint8` body, 512-aligned), `encode_mask_tiff` (grayscale `uint8` via the `tiff` crate), `write_mask_hdf5` (2D `uint8` dataset via pure-Rust `rust-hdf5`), and `encode_mask_msk` (Andy Hammersley fit2d binary bit-pack, byte-verified against fabio `Fit2dMaskImage`) — with `write_edf`/`write_tiff`/`write_msk`/`save_h5` + `save_*`/`save_mask_*` wrappers mirroring the npy path. `save_mask_dialog` opens a native rfd save picker (npy/edf/tiff/hdf5/msk filters) and dispatches by extension via the single-owner `resolve_mask_save_format` (missing→npy default, unsupported rejected — faithful to silx `save`). Remaining ◐: HDF5 save writes a fresh standalone file (silx appends in "a" mode); HDF5 dataset path defaults to `mask` (no interactive picker) |
| ◐ Partial | M | L | Mask file load (npy/edf/tif/h5/msk) | MaskToolsWidget.py:589-629 | ALL five silx formats decode+apply a 2D uint8 mask through the shared `apply_loaded_mask` crop/pad+commit owner: `read_npy`, `read_edf` (fabio `{…}` header), `read_tiff` (single-channel `uint8`), `load_h5` (`read_mask_hdf5`/`read_mask_hdf5_auto` via `rust-hdf5`, `ndims()==2` guarded), and `read_msk` (`decode_mask_msk`, fit2d bit-unpack to a binary mask, byte-verified vs fabio) — with `load_*`/`load_mask_*` wrappers. HDF5 dataset selection: `mask_datasets` enumerates 2D datasets and `load_h5_dataset` loads a chosen one (load auto-selects the single/first dataset). `load_mask_dialog` opens a native rfd open picker (npy/edf/tiff/hdf5/msk) dispatching by extension via `resolve_mask_load_format` (unknown extension rejected, faithful to silx `load`). Remaining ◐: the interactive Qt "Select a 2D dataset" picker UI is not ported (mechanism is) |
...
| ◐ | H | M | Mask save to file (npy/edf/tif/h5/msk) | `MaskToolsWidget.py:104-141 (save method: edf/tif/npy/h5/msk), 698-785 (_saveMask dialog and format handling)` | ALL five formats: `write_npy` (2D `uint8` `.npy`), `write_edf` (fabio-style EDF), `write_tiff` (grayscale `uint8` TIFF), `save_h5` (HDF5 dataset via pure-Rust `rust-hdf5`), `write_msk` (fit2d binary, `encode_mask_msk` byte-verified vs fabio) — each with `save_*`/`save_mask_*` wrappers. The file dialog is wired (`save_mask_dialog`, native rfd, npy/edf/tiff/hdf5/msk, extension-dispatched). Remaining ◐: HDF5 save writes a fresh file (silx appends in "a" mode); HDF5 path defaults to `mask` (no interactive picker). Live row 200. |
| ◐ | H | M | Mask load from file (npy/edf/tif/h5/msk) | `MaskToolsWidget.py:589-629 (load method), 630-677 (_loadMask dialog)` | ALL five formats: `read_npy`, `read_edf`, `read_tiff`, `load_h5` (`read_mask_hdf5` via `rust-hdf5`, `ndims()==2` guarded), `read_msk` (`decode_mask_msk`, fit2d bit-unpack, byte-verified vs fabio) read a 2D `uint8` mask and apply it through the shared `apply_loaded_mask` owner (crop/pad to the image, committed to undo). Dataset selection: `mask_datasets` enumerates + `load_h5_dataset` loads a named dataset (load auto-selects the first). The file dialog is wired (`load_mask_dialog`, native rfd, npy/edf/tiff/hdf5/msk, extension-dispatched). Remaining ◐: interactive Qt dataset-picker UI not ported (mechanism is). |
