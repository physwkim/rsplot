...
//! `SidmDisplay` / `OpenDisplay` / `parse_macro_args` / `next_plot_ids`).
//!
...
pub fn convert_file(input: &Path, options: &Options) -> Result<ConvertedFile, String> {
    ...
    // child implementing the shared `SidmDisplay` trait (the root too when a
...
/// The `impl SidmDisplay for Screen` delegation a hosted screen needs;
/// `trait_prefix` is `"super::"` inside a child module.
fn display_impl(trait_prefix: &str) -> String {
    format!(
        "\nimpl {trait_prefix}SidmDisplay for Screen {{\n    fn ui(&mut self, ui: &mut egui::Ui) {{\n        Screen::ui(self, ui)\n    }}\n}}\n"
    )
}
