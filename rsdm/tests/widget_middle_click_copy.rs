...
use rsdm::Engine;
use rsdm::widgets::{DrawingShape, RsdmDrawing, RsdmLabel, middle_click_copy};
use rsplot::egui;
...
fn middle_click_copies_the_stripped_pv_name() {
    ...
    let label = RsdmLabel::new(&engine, "loc://copy_lbl?type=float&init=1.0").expect("connect");
    assert!(
...
fn smallest_touched_widget_wins_the_copy() {
    ...
    let label = RsdmLabel::new(&engine, "loc://copy_small?type=float&init=2.0").expect("connect");
    assert!(
        wait_for(|| label.channel().is_connected(), Duration::from_secs(2)),
        "label channel never connected"
    );
...
fn placeholder_channels_do_not_copy() {
    let engine = Engine::new();
    let drawing = RsdmDrawing::new(&engine, "loc://adl2rsdm_shape_9", DrawingShape::Rectangle)
        .expect("connect")
...
fn address_tooltip_shows_only_while_middle_is_held() {
    ...
    let label = RsdmLabel::new(&engine, address).expect("connect");
    assert!(
        wait_for(|| label.channel().is_connected(), Duration::from_secs(2)),
        "label channel never connected"
    );
