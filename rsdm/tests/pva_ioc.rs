...
//! plugin connects directly to that server via [`rsdm::PvaPlugin::with_server`]
...
use rsdm::{Engine, PvValue, PvaPlugin};
...
fn pva_roundtrip_monitor_and_put() {
    let (engine, _server, _server_rt) = pva_engine(|source| {
        ...
        add_pv(source, "rsdm:test:pva:ao", desc, init);
    });

    let ch = engine
        .connect("pva://rsdm:test:pva:ao")
...
fn pva_enum_put_via_label_resolves_index() {
    let (engine, _server, _server_rt) = pva_engine(|source| {
        ...
        add_pv(source, "rsdm:test:pva:bo", desc, init);
    });

    let ch = engine
        .connect("pva://rsdm:test:pva:bo")
...
fn pva_subfield_path_selects_into_the_structure() {
    ...
    let (engine, _server, _server_rt) = pva_engine(|source| {
        ...
        add_pv(source, "rsdm:test:pva:tbl", desc, init);
    });

    // Column + row index → one element.
    let elem = engine
        .connect("pva://rsdm:test:pva:tbl/x/2")
    ...
    let col = engine
        .connect("pva://rsdm:test:pva:tbl/x")
...
fn pva_rpc_address_calls_the_function_and_publishes_the_result() {
    ...
    let (engine, _server, _server_rt) = pva_engine(|source| {
        ...
        source.add("rsdm:test:pva:sum", pv);
    });

    let ch = engine
        .connect("pva://rsdm:test:pva:sum?lhs=4&rhs=7&")
...
fn pva_compressed_ntndarray_decodes_lz4_end_to_end() {
    ...
    // bytes (NDPluginCodec.cpp:483); rsdm decompresses it like PyDM's
    ...
    let ch = engine
        .connect("pva://rsdm:test:pva:imgz")
...
fn pva_undecodable_compressed_ntndarray_connects_without_a_value() {
    // A compressed NTNDArray rsdm cannot decode (here: a "blosc" payload
    ...
    let ch = engine
        .connect("pva://rsdm:test:pva:imgbad")
        .expect("connect compressed ntndarray channel");
    assert!(
        wait_for(|| ch.is_connected(), Duration::from_secs(5)),
