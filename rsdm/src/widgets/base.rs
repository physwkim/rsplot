...
/// a converted screen wants only the RsDM disconnect marker — the dashed
...
/// `adl2rsdm` can convert single-sided `limits` blocks faithfully (R2-66).
...
pub struct ChannelBase {
    ...
    /// (adl2rsdm binds synthetic `loc://` addresses to MEDM widgets that carry
    /// no channel, because every rsdm widget requires one). Suppresses the
