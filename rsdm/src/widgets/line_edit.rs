//! `RsdmLineEdit` — a writable text entry.
//!
//! Ports `pydm/widgets/line_edit.py`: a single-line text field that shows the
//! channel value (formatted like [`RsdmLabel`](crate::widgets::RsdmLabel), except an enum PV shows its
...
impl RsdmLineEdit {
    ...
    /// no value has arrived. Unlike [`RsdmLabel`](crate::widgets::RsdmLabel), a line edit keeps showing the
    ...
    /// its label — unlike `RsdmLabel`. The typed index round-trips back through
...
mod tests {
    ...
    fn alarm_sensitive_content_builder_flips_the_flag() {
        ...
        let le = RsdmLineEdit::new(&engine, "loc://line_edit_alarm").expect("connect");
