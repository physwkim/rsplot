//! `RsdmSpinbox` — a numeric entry that writes a float.
//!
...
//! [`RsdmSpinbox::set_value`]; [`RsdmSpinbox::show`] is a thin egui shell.
...
mod tests {
    ...
    fn spinbox(address: &str) -> (Engine, RsdmSpinbox) {
        let engine = Engine::new();
        let spin = RsdmSpinbox::new(&engine, address).expect("connect");
        (engine, spin)
    }
    ...
    fn step_defaults_to_one_independent_of_precision() {
        ...
        // precision (spinbox.py:35); rsdm previously used 10^-precision.
    ...
    fn suffix_and_tooltip_compose_units_and_step_like_pydm() {
        ...
        // no tooltip (spinbox.py:144-145). This is what rsdm did before R3-14.
        assert_eq!(
