//! `RsdmDrawing` — a static shape with a fill, border, rotation, and optional
...
//! widgets the `adl2rsdm` converter targets): an arc is an elliptical sweep
//! within the bounds — stroked open, or filled as a pie wedge when a brush is
//! set; a polyline/polygon carries an explicit vertex list ([`RsdmDrawing::with_points`])
...
use rsplot::egui::{self, Color32, Pos2, Stroke, Vec2};
...
/// The shape drawn by a [`RsdmDrawing`] (PyDM `PyDMDrawing*` subclasses, plus
/// the MEDM `arc`/`polyline`/`polygon` shapes the `adl2rsdm` converter targets).
///
...
pub enum DrawingShape {
    ...
    /// An open polyline through the widget's [`with_points`](RsdmDrawing::with_points)
    /// vertices (MEDM `polyline`), stroked with the border pen.
    Polyline,
    /// A closed, filled polygon through the widget's
    /// [`with_points`](RsdmDrawing::with_points) vertices (MEDM `polygon`).
    Polygon,
}
...
/// geometry, so `RsdmSymbol` fills its bounds with these points rather than
...
pub(crate) fn shape_points(
    ...
    let local: Vec<(f64, f64)> = match shape {
        ...
        // derived from the box; `RsdmDrawing::paint` handles those directly and
...
mod tests {
    ...
    fn polyline_polygon_geometry_is_the_vertex_list_not_the_box() {
        ...
        let draw = RsdmDrawing::new(&engine, "loc://poly_test", DrawingShape::Polygon)
            .expect("connect")
            .with_points(vec![
                Vec2::new(0.0, 0.0),
                Vec2::new(30.0, 0.0),
