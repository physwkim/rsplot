//! `RsdmByteIndicator` — per-bit LED display of an integer value.
//!
...
//! [`RsdmByteIndicator::bit_color`]); the egui drawing is exercised by a
...
mod tests {
    ...
    fn indicator() -> RsdmByteIndicator {
        let engine = Engine::new();
        RsdmByteIndicator::new(&engine, "loc://byte_test").expect("connect")
    }
