//! `RsdmEnumButton` — pick an enum value from a row/column of buttons.
//!
...
//! [`RsdmEnumComboBox`]); the display order is the pure [`order_indices`];
//! [`RsdmEnumButton::show`] is a thin egui shell.
//!
//! [`RsdmEnumComboBox`]: crate::widgets::RsdmEnumComboBox
...
mod tests {
    ...
    fn select_writes_the_index_to_the_channel() {
        ...
        let button = RsdmEnumButton::new(&engine, "loc://enum_button_select?type=int&init=0")
            .expect("connect");
        assert!(
    ...
    fn alarm_sensitive_content_builder_flips_the_flag() {
        let engine = Engine::new();
        let button = RsdmEnumButton::new(&engine, "loc://enum_button_alarm").expect("connect");
        assert!(
