...
//!
//! rsplot's plot widget already carries a built-in right-click menu (Zoom Back /
//! Reset Zoom) and exposes
//! [`show_with_context_menu`](rsplot::PlotWidget::show_with_context_menu) to
...
//! rsplot: each plot widget owns a [`YAxisMenu`], and [`show_with_y_axis_menu`]
...
use rsplot::{Plot1D, PlotResponse, YAxis, egui};
