//! `RsdmPushButton` — write a fixed value on click.
//!
...
//! handling in [`RsdmPushButton::show`] is a thin shell over
//! [`RsdmPushButton::send_press`]. PyDM's password protection and the separate
...
mod tests {
    ...
    fn push_button_defaults_alarm_border_off_like_pydm() {
        ...
        let button =
            RsdmPushButton::new(&engine, "loc://pushbtn_border", "Step", "5").expect("connect");
    ...
    fn send_press_writes_to_the_channel() {
        ...
        let button = RsdmPushButton::new(&engine, "loc://pushbtn_send", "Step", "5")
            .expect("connect")
            .with_relative(true);
        assert!(
            wait_for(
