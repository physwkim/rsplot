//! `RsdmLabel` — a read-only value display.
//!
...
mod tests {
    ...
    fn alignment_defaults_left_and_builder_sets_it() {
        ...
        let label = RsdmLabel::new(&engine, "loc://label_align").expect("connect");
        assert_eq!(label.alignment, TextAlign::Left);
        let centered = RsdmLabel::new(&engine, "loc://label_align2")
            .expect("connect")
    ...
    fn formats_value_with_precision_and_units() {
        let engine = Engine::new();
        let label = RsdmLabel::new(&engine, "loc://label_fmt")
            .expect("connect")
            .with_precision(2)
    ...
    fn disconnected_shows_channel_address() {
        let engine = Engine::new();
        let label = RsdmLabel::new(&engine, "loc://label_disc").expect("connect");
    ...
    fn enum_value_renders_label() {
        let engine = Engine::new();
        let label = RsdmLabel::new(&engine, "loc://label_enum").expect("connect");
    ...
    fn live_value_flows_from_a_write() {
        ...
        // value below.
        let label = RsdmLabel::new(&engine, "loc://label_live?type=float&init=0").expect("connect");
