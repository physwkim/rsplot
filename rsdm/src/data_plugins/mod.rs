...
/// Whether global read-only mode is enabled via the `RSDM_READ_ONLY`
/// environment variable — the rsdm equivalent of PyDM's `pydm --read-only`
...
pub(crate) fn env_read_only() -> bool {
    read_only_flag(std::env::var("RSDM_READ_ONLY").ok().as_deref())
}

/// Parse the `RSDM_READ_ONLY` value: unset, empty and the usual "off" spellings
